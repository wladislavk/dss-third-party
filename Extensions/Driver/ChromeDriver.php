...
class ChromeDriver extends CoreDriver
{
    ...
    public function __construct($api_url = 'http://localhost:9222', HttpClient $http_client = null, $base_url, $options = [])
    {
        ...
        $jsonEndpoints = $http_client->get($api_url . '/json/list');

        try {
            $devEndpoints = json_decode($jsonEndpoints, true);
        } catch (\Exception $e) {
            $devEndpoints = [];
        }

        if (!is_array($devEndpoints) || !sizeof($devEndpoints)) {
            throw new ConnectionException('Invalid DevTools endpoint list returned from Chrome: ' . $jsonEndpoints);
        }

        $endpoint = current($devEndpoints);
        $this->browser = new ChromeBrowser($endpoint['webSocketDebuggerUrl']);

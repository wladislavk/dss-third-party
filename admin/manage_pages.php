...
if($_REQUEST["delid"] != "")
{
	$del_sql = "delete from pages where pageid='".$_REQUEST["delid"]."'";
	mysql_query($del_sql);
	...
	else
	{
		...
		?>
			<tr class="<?=$tr_class;?>">
				...
				<td valign="top">
					<a href="Javascript:;"  onclick="Javascript: loadPopup('add_page.php?ed=<?=$myarray["pageid"];?>');" class="editlink">
						<img src="images/b_edit.png" width="16" height="16" border="0" align="Edit"/>
					</a>
                    
                    <a href="<?=$_SERVER['PHP_SELF']?>?delid=<?=$myarray["pageid"];?>" onclick="javascript: return confirm('Do Your Really want to Delete?.');" class="dellink">

...
if($_REQUEST["sleep_apnea"] != "")
	$index_val = $_REQUEST["sleep_apnea"];
else
	$index_val = 0;
	
$i_val = $index_val * $rec_disp;
$sql = "select * from sleep_apnea ";
if($_GET['cid'] <> '') 
...
$my=mysql_query($sql) or die(mysql_error().$sql);
...
<div align="right">
	<button onclick="Javascript: loadPopup('add_sleep_apnea.php?cid=<?=$_GET['cid']?>');" class="addButton">
		Add New About Sleep Apnea
	</button>
	&nbsp;&nbsp;
</div>
...
<form name="sortfrm" action="<?=$_SERVER['PHP_SELF']?>?cid=<?=$_GET['cid'];?>" method="post">
<table width="98%" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF" align="center" >
	...
	<TR bgColor="#ffffff">
		<TD  align="right" colspan="15" class="bp">
			Pages:
			<?
				 paging($no_sleep_apnea,$index_val,"");
			?>
		</TD>        
	</TR>
	<? }?>
	...
	else
	{
		...
		?>
			<tr class="<?=$tr_class;?>">
				...
				<td valign="top">
					<a href="Javascript:;"  onclick="Javascript: loadPopup('add_sleep_apnea.php?ed=<?=$myarray["sleep_apneaid"];?>&cid=<?=$_GET['cid']?>');" class="editlink">
						<img src="images/b_edit.png" width="16" height="16" border="0" align="Edit"/>
					</a>
                    
                    <a href="<?=$_SERVER['PHP_SELF']?>?delid=<?=$myarray["sleep_apneaid"];?>&cid=<?=$_GET['cid']?>" onclick="javascript: return confirm('Do Your Really want to Delete?.');" class="dellink">

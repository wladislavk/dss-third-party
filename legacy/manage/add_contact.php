...
if (!empty($_POST["contactsub"]) && $_POST["contactsub"] == 1) {
    if (!empty($_POST["ed"])) {
        ...
        $let_sql = "SELECT use_letters, intro_letters FROM dental_users WHERE userid='".$db->escape( $_SESSION['docid'])."'";
        error_log($let_sql);
        $let_r = $db->getRow($let_sql);
        if ($let_r['use_letters'] && $let_r['intro_letters']) {
            $dct_sql = "SELECT physician from dental_contacttype WHERE contacttypeid=".$db->escape( $_POST["contacttypeid"]);

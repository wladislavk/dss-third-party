...
$userCompanyId = $db->getColumn("SELECT admin_company.companyid 
    FROM admin
    LEFT JOIN admin_company USING(adminid) 
    WHERE admin.adminid = '$userId'", 'companyid');
        ...
        <tr bgcolor="#FFFFFF">
            ...
            <td valign="top" class="frmdata">
                <select id="admin_access" name="admin_access" class="form-control validate">
                    ...
                    <?php if (is_super($_SESSION['admin_access'])) { ?>
                        <option value="<?= DSS_ADMIN_ACCESS_SUPER ?>" <?= $admin_access == DSS_ADMIN_ACCESS_SUPER ? 'selected' : '' ?>>Super</option>
                    <?php } ?>
                    <?php if (is_admin($_SESSION['admin_access'])) { ?>
                        <option value="<?= DSS_ADMIN_ACCESS_ADMIN ?>" <?= $admin_access == DSS_ADMIN_ACCESS_ADMIN ? 'selected' : '' ?>>Admin</option>
                    <?php } ?>
                    <?php if (is_super($_SESSION['admin_access']) || is_software($_SESSION['admin_access'])) { ?>
                        <option value="<?= DSS_ADMIN_ACCESS_BASIC ?>" <?= $admin_access == DSS_ADMIN_ACCESS_BASIC ? 'selected' : '' ?>>Basic</option>
                    <?php } ?>
                    <?php if (is_super($_SESSION['admin_access']) || is_billing_admin($_SESSION['admin_access'])) { ?>
                        <option value="<?= DSS_ADMIN_ACCESS_BILLING_ADMIN ?>" <?= $admin_access == DSS_ADMIN_ACCESS_BILLING_ADMIN ? 'selected' : '' ?>>Billing Admin</option>
                    <?php } ?>
                    <?php if (is_super($_SESSION['admin_access']) || is_billing($_SESSION['admin_access'])) { ?>
                        <option value="<?= DSS_ADMIN_ACCESS_BILLING_BASIC ?>" <?= $admin_access == DSS_ADMIN_ACCESS_BILLING_BASIC ? 'selected' : '' ?>>Billing Basic</option>
                    <?php } ?>
                    <?php if (is_super($_SESSION['admin_access']) || is_hst_admin($_SESSION['admin_access'])) { ?>
                        <option value="<?= DSS_ADMIN_ACCESS_HST_ADMIN ?>" <?= $admin_access == DSS_ADMIN_ACCESS_HST_ADMIN ? 'selected' : '' ?>>HST Admin</option>
                    <?php } ?>
                    <?php if (is_super($_SESSION['admin_access']) || is_hst($_SESSION['admin_access'])) { ?>
                        <option value="<?= DSS_ADMIN_ACCESS_HST_BASIC ?>" <?= $admin_access == DSS_ADMIN_ACCESS_HST_BASIC ? 'selected' : '' ?>>HST Basic</option>
                    <?php } ?>
                </select>
            </td>
        </tr>

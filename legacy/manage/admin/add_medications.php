...
if(!empty($_POST["mult_medicationssub"]) && $_POST["mult_medicationssub"] == 1)
{
	...
	<script type="text/javascript">
		//alert("<?php echo $msg;?>");
		parent.window.location='manage_medications.php?msg=<?php echo $msg;?>';
	</script>
	<?
	trigger_error("Die called", E_USER_ERROR);
}

if(!empty($_POST["medicationssub"]) && $_POST["medicationssub"] == 1)
{
...
<?php require_once dirname(__FILE__) . '/includes/popup_top.htm'; ?>
    ...
    <?
    $thesql = "select * from dental_medications where medicationsid='".(!empty($_REQUEST["ed"]) ? $_REQUEST["ed"] : '')."'";
    ...
    <?php }?>
    <form name="medicationsfrm" action="<?php echo $_SERVER['PHP_SELF'];?>?add=1" method="post" onSubmit="return medicationsabc(this)">
    <table class="table table-bordered table-hover">
        <tr>
            <td colspan="2" class="cat_head">
               <?php echo $but_text?> Medications 
               <?php if($medications <> "") {?>
        ...
        <tr bgcolor="#FFFFFF">
            ...
            <td valign="top" class="frmdata">
                <input type="text" name="medications" value="<?php echo $medications?>" class="form-control" /> 
                <span class="red">*</span>				
            </td>
        </tr>
        <tr bgcolor="#FFFFFF">
        ...
        <tr>
            <td  colspan="2" align="center">
                ...
                <input type="hidden" name="ed" value="<?php echo $themyarray["medicationsid"]?>" />
                <input type="submit" value="<?php echo $but_text?> Medications" class="btn btn-primary">
		<?php if($themyarray["medicationsid"] != '' && $_SESSION['admin_access']==1){ ?>
                    <a href="manage_medications.php?delid=<?php echo $themyarray["medicationsid"];?>" onclick="javascript: return confirm('Do Your Really want to Delete?.');" target="_parent" class="editdel btn btn-danger pull-right" title="DELETE">
                                                Delete
                                        </a>
		<?php } ?>
            </td>

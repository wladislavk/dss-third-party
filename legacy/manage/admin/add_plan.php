        ...
        <tr>
            <td  colspan="2" align="center">
                ...
                <?php if($themyarray["id"] != '' && $_SESSION['admin_access']==1){ ?>
                    <?php
                    $u_sql = "SELECT userid, first_name, last_name FROM dental_users where plan_id='".$db->escape($themyarray["id"])."'";
                    ...
                    if(mysqli_num_rows($u_q)>0){
                        ...
                        <a href="#" onclick="alert('This plan is associated with the following users and cannot be deleted: <?php echo  $names; ?>'); return false;" class="editdel btn btn-danger pull-right" title="DELETE">
                            Delete
                        </a>
                    <?php }else{ ?>
                        <a href="manage_plans.php?delid=<?php echo $themyarray["id"];?>" onclick="return confirm('Do Your Really want to Delete?.');" target="_parent" class="editdel btn btn-danger pull-right" title="DELETE">
                            Delete
                        </a>
                        <?php
                    }

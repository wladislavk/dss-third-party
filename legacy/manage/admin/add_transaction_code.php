...
if(!empty($_POST["transaction_codesub"]) && $_POST["transaction_codesub"] == 1) {
    $sel_check = "select * from dental_transaction_code where default_code=1 and transaction_code = '".s_for($_POST["transaction_code"])."' and transaction_codeid != '".s_for($_POST['ed'])."'";
    $query_check = mysqli_query($con,$sel_check);
    ...
    if(!empty($query_check) && mysqli_num_rows($query_check) > 0) {
        ...
        if($_POST["ed"] != "") {
            $ed_sql = "update dental_transaction_code set 
                transaction_code = '".s_for($_POST["transaction_code"])."', 
                ...
                type = '".s_for($_POST["type"])."' 
                where transaction_codeid='".$_POST["ed"]."'";
            ...
            $ins_sql = "insert into dental_transaction_code set 
                transaction_code = '".s_for($_POST["transaction_code"])."', 
                place='".s_for($_POST['place'])."', 
                modifier_code_1 = '".s_for($_POST['modifier_code_1'])."',                                
                modifier_code_2 = '".s_for($_POST['modifier_code_2'])."',
        ...
        <tr>
            <td  colspan="2" align="center">
                ...
                <?php if($themyarray["transaction_codeid"] != '' && $_SESSION['admin_access']==1){ ?>
                    <a href="manage_transaction_code.php?delid=<?php echo $themyarray["transaction_codeid"];?>" onclick="return confirm('Do Your Really want to Delete?.');" target="_parent" class="editdel btn btn-danger pull-right" title="DELETE">
                        Delete
                    </a>
                <?php } ?>
            </td>

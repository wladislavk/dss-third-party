...
if(is_super($_SESSION['admin_access'])){
    $sql = "SELECT du.userid, du.username, 
        ...
        LEFT JOIN (
            SELECT COALESCE(sum(dl.percase_amount),0) as ledger_amount, pi.docid 
            FROM dental_percase_invoice pi 
            LEFT JOIN dental_ledger dl on pi.id=dl.percase_invoice
            GROUP BY pi.docid
        ) i ON i.docid=du.userid
        LEFT JOIN (
            SELECT COALESCE(sum(pi.monthly_fee_amount),0) as monthly_amount, pi.docid 
            FROM dental_percase_invoice pi 
            GROUP BY pi.docid
        ) i1 ON i1.docid=du.userid
        LEFT JOIN (
            SELECT COALESCE(sum(dle.percase_amount),0) as extra_amount, pi.docid 
            FROM dental_percase_invoice pi 
            LEFT JOIN dental_percase_invoice_extra dle ON dle.percase_invoice=pi.id
            GROUP BY pi.docid
        ) i2 ON i2.docid=du.userid
        LEFT JOIN (
            SELECT COALESCE(sum(ip.invoice_amount), 0) as vob_amount, pi.docid 
            FROM dental_percase_invoice pi 
            LEFT JOIN dental_insurance_preauth ip ON ip.invoice_id = pi.id
            GROUP BY pi.docid
        ) i3 ON i3.docid=du.userid
        WHERE du.docid=0 ";
}
...
<div class="row">
    ...
    <?php
    $count_sql = "SELECT count(du.userid) num_paid 
        from dental_users du
        LEFT JOIN (
            SELECT COALESCE(sum(dl.percase_amount),0) as ledger_amount, pi.docid 
            FROM dental_percase_invoice pi
        ...
        WHERE du.docid=0 
        AND status=1 
        AND (i.ledger_amount + i1.monthly_amount + i2.extra_amount + i3.vob_amount) > 0";
    $count_q = mysqli_query($con,$count_sql);
    $count_r = mysqli_fetch_assoc($count_q);
    ?>
    ...
    $count_sql = "SELECT count(du.userid) num_paid 
        ...
        WHERE du.docid=0 
        AND status=1 
        AND COALESCE((i.ledger_amount + i1.monthly_amount + i2.extra_amount + i3.vob_amount), 0) <= 0";
    $count_q = mysqli_query($con,$count_sql);
    $count_r = mysqli_fetch_assoc($count_q);
    ?>
    ...
    <br />
    <?php
    $count_sql = "SELECT COALESCE(sum(dc.amount),0) cc_paid 
        from dental_charge dc
    ...
    <?php
    $total_charge = 0;
    $sql = "SELECT id, monthly_fee_amount 
        FROM dental_percase_invoice pi
    ...
    while($myarray = mysqli_fetch_assoc($q)){
        ...
        $case_sql = "SELECT percase_name, percase_date as start_date, '' as end_date, percase_amount, ledgerid 
                ...
                AND dl.docid='".(!empty($myarray['docid']) ? $myarray['docid'] : '')."' 
                AND dl.percase_invoice='".(!empty($myarray['id']) ? $myarray['id'] : '')."'
            UNION
                SELECT percase_name, percase_date, '', percase_amount, id 
                FROM dental_percase_invoice_extra dl
                WHERE dl.percase_invoice='".$myarray['id']."'
            UNION
...
<table class="sort_table table table-bordered table-hover" id="tracking_table">
    ...
    <tr class="tr_bg_h">
        ...
        <th class="col_head">Billing Company</th>
        <th class="col_head"># Staff</th>
        <th class="col_head"># Patients</th>
        <th class="col_head"># Contacts</th>
        <th class="col_head">Act. Code</th>
        <th class="col_head">Plan</th>
        <th class="col_head">Activated</th>
        <th class="col_head">Duration</th>
        <th class="col_head">$ Paid</th>

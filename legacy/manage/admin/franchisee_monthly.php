...
<table class="sort_table table table-bordered table-hover" id="monthly_table">
    <thead>
    <tr class="tr_bg_h">
        <th valign="top" class="col_head">Username&nbsp;&nbsp;</th>
        <th valign="top" class="col_head">Company</th>
        <th valign="top" class="col_head">Name</th>
        <th valign="top" class="col_head">Pt. Screened</th>
        <th valign="top" class="col_head">By user&nbsp;&nbsp;</th>
        <th valign="top" class="col_head">Completed Sleep Studies</th>
        <th valign="top" class="col_head">Consult&nbsp;&nbsp;</th>
        <th valign="top" class="col_head">Impressions&nbsp;&nbsp;</th>
        <th valign="top" class="col_head">Device Delivery&nbsp;&nbsp;</th>
        <th valign="top" class="col_head">Letters Sent</th>
        <th valign="top" class="col_head">VOBs Completed</th>
        <th valign="top" class="col_head">Ins. Claims Sent</th>
        <th valign="top" class="col_head">Ins. Claims Paid</th>
    </tr>
    </thead>
    <tbody>
    <?php if ($total_rec == 0) { ?>
        ...
        while($myarray = mysqli_fetch_array($my)) {
            ...
            $dd_sql = "SELECT count(i.id) as num_dd 
                FROM dental_flow_pg2_info i
                JOIN dental_patients p ON p.patientid = i.patientid
                WHERE i.segmentid=7
                AND p.docid='".$db->escape($myarray['userid'])."'
            ...
            $ins_sent_sql = "SELECT count(h.id) as num_sent 
                FROM dental_insurance i
                JOIN dental_insurance_status_history h ON i.insuranceid=h.insuranceid
                WHERE i.docid='".$db->escape($myarray['userid'])."'
                AND (h.status = '".$db->escape(DSS_CLAIM_SENT)."' OR h.status = '".$db->escape(DSS_CLAIM_SEC_SENT)."')
            ...
            $ins_paid_sql = "SELECT count(h.id) as num_paid 
                FROM dental_insurance i
                JOIN dental_insurance_status_history h ON i.insuranceid=h.insuranceid
                WHERE i.docid='".$db->escape($myarray['userid'])."'
                AND h.status IN (
                    ".$db->escape(DSS_CLAIM_PAID_INSURANCE).",

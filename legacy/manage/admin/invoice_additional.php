...
if(isset($_GET['show']) && $_GET['show']=='1'){
    ...
    $sql = "SELECT 
        ...
        WHERE du.status=1 
        AND du.docid=0
        AND ((SELECT i2.monthly_fee_date FROM dental_percase_invoice i2 WHERE i2.docid=du.userid AND i2.invoice_type='".$db->escape(DSS_INVOICE_TYPE_SU_FO)."' ORDER BY i2.monthly_fee_date DESC LIMIT 1) < DATE_SUB(now(), INTERVAL 1 MONTH) OR ((SELECT i2.monthly_fee_date FROM dental_percase_invoice i2 WHERE i2.docid=du.userid AND i2.invoice_type='".$db->escape(DSS_INVOICE_TYPE_SU_FO)."' ORDER BY i2.monthly_fee_date DESC LIMIT 1) IS NULL AND DATE_ADD(du.adddate, INTERVAL plan.trial_period DAY) < now()))
    ";
    ...
    if(isset($_GET['show']) && $_GET['show']=='all'){
        ...
        $sql .= " AND (SELECT COUNT(*) as num_inv FROM dental_percase_invoice WHERE docid=du.userid AND status = '".DSS_INVOICE_PENDING."') != 0
...
if(mysqli_num_rows($q) > 0){
    ...
    $invoice_id = $r['id'];
    $efile_sql = "SELECT dp.firstname, dp.lastname, e.id, e.adddate 
    ...
    $ec_sql = "SELECT count(*) as total_ec, MIN(e.adddate) as start_date, MAX(e.adddate) as end_date 
    ...
    $enroll_sql = "SELECT count(*) as total_enrollments, MIN(e.adddate) as start_date, MAX(e.adddate) as end_date 
...
$producer_sql = "SELECT count(*) as total_producers 
    FROM dental_users u
    WHERE u.docid = '".$db->escape((!empty($user['id']) ? $user['id'] : ''))."'
    and u.status = 1 
    and u.producer=1";
    ...
    if(mysqli_num_rows($q) > 0){
        ...
        if(isset($_POST['amount_monthly'])){
            $in_sql .= ", monthly_fee_date = '".$db->escape(date('Y-m-d', strtotime($_POST['monthly_date'])))."',
                monthly_fee_amount = '".$db->escape($_POST['amount_monthly'])."' ";
        ...
        if(isset($_POST['fax_desc'])){
            ...
            $up_sql = "UPDATE dental_faxes SET
                status = '1'
                WHERE fax_invoice_id = '".$fax['fax_invoice_id']."' AND docid='".$db->escape($_REQUEST['docid'])."'";
            mysqli_query($con,$up_sql);
            ...
            $up_sql = "UPDATE dental_fax_invoice SET
                invoice_id = '".$db->escape($i_id)."'
                WHERE invoice_id = '".$invoice_id."'
                AND id != '".$db->escape( $fax_invoice_id)."'"; //DO NOT UPDATE FREE FAX INVOICE
            mysqli_query($con,$up_sql);
        }
    ...
    <div class="panel-body">
        ...
        <form name="sortfrm" id="invoice" action="<?php echo $_SERVER['PHP_SELF']?>?show=<?php echo (!empty($_GET['show']) ? $_GET['show'] : ''); ?>&company=<?php echo (!empty($_GET['company']) ? $_GET['company'] : '');?>&bill=<?php echo (!empty($_GET['bill']) ? $_GET['bill'] : '');?>&docid=<?php echo  (!empty($_GET['docid']) ? $_GET['docid'] : ''); ?>&uid=<?php echo (!empty($_GET['uid']) ? $_GET['uid'] : '');?>&cc=<?php echo  ($count_current); ?>&ci=<?php echo  $count_invoices; ?>" method="post">
            ...
            <table id="invoice_table" class="table table-bordered table-hover">
                ...
                <?php } ?>
                <?php if (!empty($case_q)) {
                    while ($case = mysqli_fetch_array($case_q)) { ?>

...
if (isset($_GET['upstatus'])) {
    ...
    if ($allowBO) {
        ...
        if ($proposedStatusName && $proposedStatusName != $oldStatusName) {
            ...
            $newStatus = ClaimFormData::isPrimary($oldStatus) ? $possibleStatuses[$proposedStatusName][0] : $possibleStatuses[$proposedStatusName][1];
    ...
    <form name="sortfrm" action="<?php echo $_SERVER['PHP_SELF']?>" method="get">
        ...
        <?php if (!empty($_REQUEST['fid'])) { ?>
            ...
            <select name="pid">
                <option value="">Any</option>
                <?php
                $patients = get_patients($_REQUEST['fid']);
                while ($row = mysqli_fetch_array($patients)) {
                    $selected = ($row['patientid'] == $_REQUEST['pid']) ? 'selected' : ''; ?>
            ...
            while ($myarray = mysqli_fetch_array($my)) { ?>
                <tr class="<?php echo (isset($tr_class)) ? $tr_class : '';?>">
                    ...
                    <td>
                        ...
                        $updatedAfterEResponse = $eResponse ? strtotime($latestStatusChange['adddate']) > strtotime($eResponse['data']['adddate']) : true;
                    ...
                    <td valign="top">
                        ...
                        $primary_link = 'insurance_claim' . ($myarray['primary_claim_version'] != 1 ? '_eligible' : '_v2') . '.php?' . http_build_query([
                            'insid' => $primaryClaimId,
                            'fid_filter' => $fid,
                            'pid_filter' => $pid,
                            'pid' => $myarray['patientid'],
                        ]);
                        $secondary_link = 'insurance_claim' . ($myarray['secondary_claim_version'] != 1 ? '_eligible' : '_v2') . '.php?' . http_build_query([
                        ...
                        if (mysqli_num_rows($reference_id_query)) {
                            ...
                            if ($reference_id != "" && !in_array($myarray['status'], $paid_statuses)) {
                                ...
                                <?php
                            } elseif ($reference_id != "" && in_array($myarray['status'], $paid_statuses)) {
                        ...
                        if ($myarray['status'] == DSS_CLAIM_DISPUTE || $myarray['status'] == DSS_CLAIM_PATIENT_DISPUTE) {

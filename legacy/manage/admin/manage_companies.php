			...
			<tr <?php echo  ($myarray['status']==2)?'class="warning"':''; ?>>
				...
				<td valign="top">
				  <?php 
					...
					$u_sql = "SELECT userid FROM dental_users WHERE docid = 0 AND billing_company_id='".$db->escape($myarray["id"])."'";
					...
					<?php
                                        ...
                                        $u_sql = "SELECT u.userid FROM dental_users u join dental_user_hst_company uhc ON uhc.userid=u.userid WHERE u.docid = 0 AND uhc.companyid='".$db->escape($myarray["id"])."'";
                                        $u_q = mysqli_query($con,$u_sql);
                                        $num_users = mysqli_num_rows($u_q);
                                        ?>
                                        ...
                                        $u_sql = "SELECT u.userid FROM dental_users u join dental_user_company uc ON uc.userid=u.userid WHERE u.docid = 0 AND uc.companyid='".$db->escape($myarray["id"])."'";
                                        $u_q = mysqli_query($con,$u_sql);
                                        $num_users = mysqli_num_rows($u_q);
                                        ?>

...
if (is_super($_SESSION['admin_access'])) {
    ...
    $sql = "SELECT
        ...
        JOIN dental_users users2 ON hst.user_id = users2.userid
        LEFT JOIN dental_users users3 ON hst.authorized_id = users3.userid
        JOIN dental_user_hst_company uhc ON uhc.userid = users.userid AND uhc.companyid = hst.company_id AND hst.company_id = '$adminCompanyId'
    ...
    $sql = "SELECT
        ...
        FROM dental_hst hst
        LEFT JOIN dental_patients p ON hst.patient_id = p.patientid
        LEFT JOIN dental_user_company uc ON uc.userid = p.docid AND uc.companyid = '$adminCompanyId'
...
<form name="pagefrm" action="<?php echo $_SERVER['PHP_SELF']?>" method="post">
    <table class="table table-bordered table-hover">
        ...
        <?php } ?>
        <?php
        $sort_qs = $_SERVER['PHP_SELF'] . "?fid=" . $fid . "&pid=" . $pid . "&status=" . $_REQUEST['status'] . "&sort_by=%s&sort_dir=%s";
        ?>
        <tr class="tr_bg_h">
            ...
            while($myarray = mysqli_fetch_array($my)) {
                ...
                if ($myarray['status'] == DSS_HST_CANCELED) {
                    ...
                    if ($cancelDate || $cancelerName) {
                        $title = 'HST canceled' . ($cancelDate ? " on $cancelDate" : '') . ($cancelerName ? " by $cancelerName" : '');
                    }
                }
                ?>

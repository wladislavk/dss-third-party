...
if ($status == 'pending') {
    if(is_super($_SESSION['admin_access'])) {
        ...
        $letters_query = "SELECT dental_letters.letterid, 
            ...
            JOIN dental_users u ON u.userid = dental_letters.docid
            LEFT JOIN dental_patients on dental_letters.patientid=dental_patients.patientid 
            WHERE u.billing_company_id='".$db->escape($_SESSION['admincompanyid'])."' 
            AND ((dental_letters.status = '1' AND dental_letters.delivered=0) 
                OR (dental_letters.delivered = '1' AND dental_letters.mailed_date IS NULL)
            ) 
            AND dental_letters.deleted = '0' 
            AND dental_letters.templateid LIKE '".$filter."' ".$doc_filter." 
...
if ($status == 'sent') {
    if(is_super($_SESSION['admin_access'])){
        $letters_query = "SELECT dental_letters.letterid, 
            ...
            LEFT JOIN dental_patients on dental_letters.patientid=dental_patients.patientid 
            WHERE dental_letters.mailed_date IS NOT NULL 
            AND dental_letters.deleted = '0' 
            AND dental_letters.templateid LIKE '".$filter."' ".$doc_filter." 
            AND u.user_type = '".DSS_USER_TYPE_FRANCHISEE."' 
            ORDER BY dental_letters.letterid ASC;";
...
foreach ($dental_letters as $key => $letter) {
    ...
    if ($status == "pending") {
        ...
        if ($age_in_days > 7) {
            $dental_letters[$key]['bg'] = "danger";
        }
        if ($age_in_days <= 7) {
            $dental_letters[$key]['bg'] = "warning";
        }
    } elseif ($status == "sent") {
...
<div style="clear:both;">
    <table id="letters-table" class="table table-bordered table-hover">
        <tr class="tr_bg_h">
            ...
            <td class="col_head <?php echo ($_REQUEST['sort'] == 'subject')?'arrow_'.strtolower($_REQUEST['sortdir']):''; ?>"><a href="manage_letters.php?status=<?php echo $status;?>&page=<?php echo $page;?>&filter=<?php echo $filter;?>&sort=subject&sortdir=<?php echo ($_REQUEST['sort']=='subject'&&$_REQUEST['sortdir']=='ASC')?'DESC':'ASC'; ?>">Correspondence</a></td>

...
if(is_super($_SESSION['admin_access'])){
    $sql = "SELECT c.*, p.name as plan_name,
        (SELECT COUNT(i.id) FROM dental_percase_invoice i WHERE i.companyid=c.id AND i.invoice_type='".$db->escape(DSS_INVOICE_TYPE_SU_BC)."') AS num_invoices,
        ...
        WHERE c.status=1 AND ((SELECT i2.monthly_fee_date FROM dental_percase_invoice i2 WHERE i2.companyid=c.id AND i2.invoice_type='".$db->escape(DSS_INVOICE_TYPE_SU_BC)."' ORDER BY i2.monthly_fee_date DESC LIMIT 1) < DATE_SUB(now(), INTERVAL 1 MONTH) OR 
        ((SELECT i2.monthly_fee_date FROM dental_percase_invoice i2 WHERE i2.companyid=c.id AND i2.invoice_type='".$db->escape(DSS_INVOICE_TYPE_SU_BC)."' ORDER BY i2.monthly_fee_date DESC LIMIT 1) IS NULL AND DATE_ADD(c.adddate, INTERVAL p.trial_period DAY) < now()))
        ";
}else{
    $sql = "SELECT du.*, c.name AS company_name, p.name as plan_name,
        ...
        (SELECT COUNT(*) AS num_trxn 
            FROM dental_ledger dl 
            JOIN dental_patients dp ON dl.patientid=dp.patientid
            WHERE dl.transaction_code='E0486' 
            AND dl.docid=du.userid 
            AND dl.percase_status = '".DSS_PERCASE_PENDING."') AS num_case, 
        (SELECT COUNT(*) AS num_trxn 
            FROM dental_ledger dl 
            JOIN dental_patients dp ON dl.patientid=dp.patientid
            WHERE dl.transaction_code='E0486' 
            AND dl.docid=du.userid 
            AND dl.service_date > DATE_SUB(now(), INTERVAL 30 DAY)) as num_case30 
...
<div class="row text-center">
    ...
    <div class="col-md-4 text-center">
        <a class="btn btn-sm btn-info" href="invoice_bo_additional.php?show=all&bill=0<?php echo  (isset($_GET['company']) && $_GET['company'] != "")?"&company=".$_GET['company']:""; ?>">
            Invoice All
            <span class="glyphicon glyphicon-envelope"></span>
        </a>
        <a class="btn btn-sm btn-primary" href="invoice_bo_additional.php?show=all&bill=1<?php echo  (isset($_GET['company']) && $_GET['company'] != "")?"&company=".$_GET['company']:""; ?>">
            Invoice And Bill All
            <span class="glyphicon glyphicon-usd"></span>
        </a>
    </div>
    <div class="col-md-4 text-right">
        <a class="btn btn-sm btn-info" href="invoice_bo_additional.php?bill=0<?php echo  (isset($_GET['company']) && $_GET['company'] != "")?"&company=".$_GET['company']:""; ?>">
            Invoice Additional
            <span class="glyphicon glyphicon-envelope"></span>
        </a>
        <a class="btn btn-sm btn-primary" href="invoice_bo_additional.php?bill=1<?php echo  (isset($_GET['company']) && $_GET['company'] != "")?"&company=".$_GET['company']:""; ?>">
        ...
        <?php if(mysqli_num_rows($my) == 0) { ?>
            ...
            while($myarray = mysqli_fetch_array($my)) { ?>
                <tr class="status_<?php echo  $myarray["status"]; ?>">
                    ...
                    <td valign="top" style="color:#f00;font-weight:bold;text-align:center;">
                        <?php echo st($myarray["num_case"]); ?>
                    </td>
                    <td valign="top">
                        <?php echo st($myarray["num_invoices"]);?>
                    </td>
                    <td valign="top" align="center">
                        <a href="manage_percase_bo_invoice_history.php?companyid=<?php echo $myarray["id"];?>">History</a>
                    </td>
                    <td valign="top">

...
$i_val = $index_val * $rec_disp;
  $sql = "SELECT du.*, c.name AS company_name, c.id AS company_id, p.name as plan_name,
		...
		du.status=1 AND du.docid=0 AND ((SELECT i2.monthly_fee_date FROM dental_percase_invoice i2 WHERE i2.docid=du.userid AND i2.invoice_type='".$db->escape( DSS_INVOICE_TYPE_BC_FO)."' ORDER BY i2.monthly_fee_date DESC LIMIT 1) < DATE_SUB(now(), INTERVAL 1 MONTH) OR 

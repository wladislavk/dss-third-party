...
$sql = "SELECT pi.* 
    ...
    AND pi.docid=".$db->escape($_GET['docid'])." 
...
<div class="page-header">
    <h2>Invoice History
        <small>
            - <?php echo  $doc['first_name']; ?> <?php echo  $doc['last_name']; ?>
            <?php if(!empty($last4)){ ?>
                - Current Card Ending
                <?php echo $last4;
            } ?>
        ...
        <?php if(mysqli_num_rows($my) == 0) { ?>
            ...
            while($myarray = mysqli_fetch_array($my)) {
                ...
                $case_sql = "SELECT percase_name, percase_date as start_date, '' as end_date, percase_amount, ledgerid 
                    ...
                    AND dl.docid='".$myarray['docid']."' 
                    AND dl.percase_invoice='".$myarray['id']."'
                UNION
                    SELECT percase_name, percase_date, '', percase_amount, id 
                    FROM dental_claim_electronic e 
                    WHERE e.percase_invoice='".$myarray['id']."'
                UNION
                    SELECT percase_name, percase_date, '', percase_amount, insuranceid 
                    FROM dental_insurance i
                    WHERE i.percase_invoice='".$myarray['id']."'
                UNION
                    SELECT percase_name, percase_date, '', percase_amount, id 
                ...
                UNION
                    ...
                    WHERE invoice_id='".$myarray['id']."'
                UNION
                    SELECT new_fee_desc, new_fee_date, '', new_fee_amount, patientid 
                    FROM dental_patients
                    WHERE new_fee_invoice_id='".$myarray['id']."'
                ";
                $case_q = mysqli_query($con,$case_sql);
                while($case_r = mysqli_fetch_assoc($case_q)){
                    $total_charge += $case_r['percase_amount'];
                }
                $case_sql = "SELECT COUNT(*) AS num_trxn FROM dental_ledger dl WHERE dl.percase_invoice='".$myarray['id']."'";
                ?>
                <style type="text/css">
                    tr.status_2 td{
                        color:#f33;
...
<?php
$charge_sql = "SELECT * FROM dental_charge WHERE userid='".$db->escape($_GET['docid'])."'";
            ...
            while($charge_r = mysqli_fetch_array($charge_q)) { ?>
                <tr>
                    ...
                    <td valign="top" style="font-weight:bold;">
                        $<?php echo st($charge_r["amount"]); ?>
                    </td>
                    <td valign="top" style="font-weight:bold;">
                        <?php
                        $r_sql = "SELECT SUM(amount) refund FROM dental_refund WHERE charge_id='".$db->escape($charge_r['id'])."'";
                        $r_q = mysqli_query($con,$r_sql);
                        $refund = mysqli_fetch_assoc($r_q);
                        ?>
                        $<?php echo number_format($refund["refund"], 2); ?>
                    </td>
                    <td valign="top" style="font-weight:bold;">
                        <a href="https://manage.stripe.com/customers/<?php echo st($charge_r["stripe_customer"]); ?>" target="_blank">
                            <?php echo st($charge_r["stripe_customer"]); ?>
                        </a>
                    </td>
                    <td valign="top" style="font-weight:bold;">
                        <a href="https://manage.stripe.com/payments/<?php echo st($charge_r["stripe_charge"]); ?>" target="_blank">
                            <?php echo st($charge_r["stripe_charge"]); ?>
                        </a>
                    </td>
                    <td valign="top">
                        <?php

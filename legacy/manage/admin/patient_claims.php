...
if(isset($_REQUEST['sendid'])){
    if($_REQUEST['sendid']!=''){
        ...
        $send_sql = "SELECT i.*, f.description AS dispute_description 
...
<div class="page-header">
    Manage Claims
    <?php if ($isSuperAdmin) { ?>
        <a class="btn btn-xs <?= $specialFilter == 'front' ? 'btn-info active' : 'btn-default' ?>" href="/manage/admin/patient_claims.php?pid=<?= $patientId ?>&amp;filed_by=front">FO Claims</a>
        <a class="btn btn-xs <?= !$specialFilter ? 'btn-info active' : 'btn-default' ?>" href="/manage/admin/patient_claims.php?pid=<?= $patientId ?>">BO Claims</a>
        <a class="btn btn-xs <?= $specialFilter == 'both' ? 'btn-info active' : 'btn-default' ?>" href="/manage/admin/patient_claims.php?pid=<?= $patientId ?>&amp;filed_by=both">Both</a>
    <?php } ?>
</div>
...
<form name="pagefrm" action="<?=$_SERVER['PHP_SELF']?>" method="post">
    <table class="table table-bordered table-hover">
        ...
        <?php }?>
        <?php
        $sort_qs = $_SERVER['PHP_SELF'] . "?fid=" . $fid . "&pid=" . $pid . "&status=" . ((isset($_REQUEST['status']))?$_REQUEST['status']:'') . "&sort_by=%s&sort_dir=%s" . ($specialFilter ? "&filed_by=$specialFilter" : '');
        ?>
        <tr class="tr_bg_h">
                ...
                <tr class="<?= (isset($tr_class))?$tr_class:'';?>">
                        ...
                        <td valign="top">
                            <p class="input-group text-center"
                                ...
                                <span class="input-group-btn bo-status-switch">
                                    <button <?= $isPending ? 'disabled' : '' ?> class="filed-by-fo btn btn-xs <?= !$myarray['filed_by_bo'] ? 'btn-info active' : 'btn-default' ?>" type="button" data-claim-id="<?= $myarray['insuranceid'] ?>"<?= $myarray['filed_by_bo'] ? 'title="Mark the claim as filed by the FO"' : '' ?>>FO</button>
                                    <button <?= $isPending ? 'disabled' : '' ?> class="filed-by-bo btn btn-xs <?= $myarray['filed_by_bo'] ? 'btn-info active' : 'btn-default' ?>" type="button" data-claim-id="<?= $myarray['insuranceid'] ?>"<?= !$myarray['filed_by_bo'] ? 'title="Mark the claim as filed by the BO"' : '' ?>>BO</button>
                                </span>
                            </p>
                        </td>
                    <?php } ?>
                    <td valign="top">
                        ...
                        if(mysqli_num_rows($reference_id_query)){
                            ...
                            if($reference_id != "" && !in_array($myarray['status'], $paid_statuses)){
                                ...
                                ?>
                                <a href="<?= $update_claim_url ?>" onclick="loadPopup('<?= $update_claim_url ?>&amp;embed=1'); return false;" class="btn btn-primary btn-sm" >Check Status</a>
                                <?php
                                ...
                                ?>
                                <a href="<?= $payment_status_url ?>" onclick="loadPopup('<?= $payment_status_url ?>&amp;embed=1'); return false;" class="btn btn-primary btn-sm" >Check Payment Status</a>
                                <?php
                            }
                        }
                        ?>
                        <?php if($myarray["status"] == DSS_CLAIM_PENDING || $myarray["status"] == DSS_CLAIM_REJECTED){ ?>
                            <a href="<?= $primary_link ?>" class="btn btn-primary btn-sm">
                                Edit
                        ...
                        <?php }elseif($myarray["status"] == DSS_CLAIM_SEC_PENDING){ ?>
                            ...
                            <a href="<?= $primary_link ?>" class="btn btn-primary btn-sm">View Primary <span class="glyphicon glyphicon-pencil"></span></a>
                    ...
                    <td>
                        ...
                        if($myarray['status'] == DSS_CLAIM_SENT || $myarray['status'] == DSS_CLAIM_DISPUTE || $myarray['status'] == DSS_CLAIM_PAID_INSURANCE || $myarray['status'] == DSS_CLAIM_PENDING || $myarray['status'] == DSS_CLAIM_PAID_PATIENT || $myarray['status'] == DSS_CLAIM_REJECTED || $myarray['status'] == DSS_CLAIM_PATIENT_DISPUTE  || $myarray['status'] == DSS_CLAIM_EFILE_ACCEPTED  ){ ?>
                            <input type="checkbox" class="mailed_chk" value="<?= $myarray['insuranceid']; ?>"
                            <?php
                            ...
                            <input type="checkbox" class="sec_mailed_chk" value="<?= $myarray['insuranceid']; ?>" <?php echo ($myarray['sec_mailed_date'] !='')?'checked="checked"':'';?> />
                        <?php } ?>
                    </td>
                </tr>
                <?php

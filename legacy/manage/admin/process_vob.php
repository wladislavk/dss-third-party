...
$sql = "SELECT preauth.patient_id
    FROM dental_insurance_preauth preauth
    WHERE preauth.id = " . $_REQUEST['ed'];
$result = mysqli_query($con, $sql);
$pid = mysql_result($result, 0);

if (isset($_REQUEST['ed'])) {
    // load preauth
    $sql = "SELECT preauth.*, id.ins_diagnosis, pcp.salutation as 'pcp_salutation', pcp.firstname as 'pcp_firstname', pcp.lastname as 'pcp_lastname', pcp.phone1 as 'pcp_phone1', p.patientid as 'patientid' 
        FROM dental_insurance_preauth preauth 
    ...
    $sql = "SELECT
        ...
        WHERE p.patientid = ".$preauth['patientid'];
    ...
    <script type='text/javascript'>
        parent.window.location='manage_vobs.php?msg=<?= $msg ?>';
    </script>
    <?php
}
...
<form name="preauth_form" action="<?=$_SERVER['PHP_SELF'];?>" method="post" onSubmit="return validatePreAuthForm(this)">
    <table class="table table-bordered table-hover">
        ...
        <tr bgcolor="#FFFFFF">
            ...
            <td valign="top" class="frmdata">
                from
                <input id="ins_cal_year_start" type="text" name="ins_cal_year_start" value="<?=$preauth['ins_cal_year_start']?>" onchange="validateDate('ins_cal_year_start');"class="tbox calendar" style="width:125px" <?=$disabled?>/>
                to
                <input id="ins_cal_year_end" type="text" name="ins_cal_year_end" value="<?=$preauth['ins_cal_year_end']?>" onchange="validateDate('ins_cal_year_end');" class="tbox calendar" style="width:125px" <?=$disabled?>/>
                <span class="red">*</span>
        ...
        <tr bgcolor="#FFFFFF">
            ...
            <td valign="top" class="frmdata">
                <?php
                $yes_checked = ($preauth['trxn_code_covered'] == '1') ? 'CHECKED' : '';
                $no_checked = ($preauth['trxn_code_covered'] != '1') ? 'CHECKED' : '';
                ?>
        ...
        <tr bgcolor="#FFFFFF" class="covered-row">
            ...
            <td valign="top" class="frmdata">
                <?php
                $yes_checked = ($preauth['has_out_of_network_benefits'] == '1') ? 'CHECKED="true"' : '';
                $no_checked  = ($preauth['has_out_of_network_benefits'] != '1') ? 'CHECKED="true"' : '';
                ?>
                ...
                <div id="has_out_of_network_benefits_no" class="sub-question">
                    Is this an HMO?
                    <?php
                    $yes_checked = ($preauth['is_hmo'] == '1') ? 'CHECKED' : '';
                    $no_checked  = ($preauth['is_hmo'] != '1') ? 'CHECKED' : '';
                    ?>
                    ...
                    <div id="is_hmo_yes" class="sub-question">
                        ...
                        <br/>
                        <?php
                        if (empty($preauth['hmo_date_called'])) {
                            $preauth['hmo_date_called'] = date('m/d/Y');
                        } ?>
        ...
        <tr bgcolor="#FFFFFF" class="covered-row">
            ...
            <td valign="top" class="frmdata">
                <?php
                $yes_checked = ($preauth['network_benefits'] == '1') ? 'CHECKED="true"' : '';
                $no_checked = ($preauth['network_benefits'] != '1') ? 'CHECKED="true"' : '';
                ?>
        ...
        <tr bgcolor="#FFFFFF" class="covered-row">
            ...
            <td valign="top" class="frmdata">
                <?php
                $patient_checked = ($preauth['deductible_from'] == '1') ? 'CHECKED="true"' : '';
                $family_checked  = ($preauth['deductible_from'] != '1') ? 'CHECKED="true"' : '';
                ?>
        ...
        <tr bgcolor="#FFFFFF" class="covered-row">
            ...
            <td valign="top" class="frmdata">
                <?php
                $yes_checked = ($preauth['out_of_pocket_met'] == '1') ? 'CHECKED' : '';
                $no_checked  = ($preauth['out_of_pocket_met'] != '1') ? 'CHECKED' : '';
                ?>
        ...
        <tr bgcolor="#FFFFFF" class="covered-row">
            ...
            <td valign="top" class="frmdata">
                <?php
                $yes_checked = ($preauth['is_pre_auth_required'] == '1') ? 'CHECKED' : '';
                $no_checked  = ($preauth['is_pre_auth_required'] != '1') ? 'CHECKED' : '';
                ?>
            ...
            <td colspan="2" align="center">
                ...
                <?php if (!$is_complete && !$is_rejected) { ?>
                    ...
                    <div id="reject_reason_div" <?= ($preauth['status'] == DSS_PREAUTH_REJECTED) ? '' : 'style="display:none;"'; ?>>
                        <label>VOB will be REJECTED and franchisee will be notified.  Please list the reasons for rejection.</label>
                        <br />
                        <textarea id="reject_reason" name="reject_reason"><?= $preauth['reject_reason']; ?></textarea>
                ...
                Mark Complete <input type="checkbox" name="complete" value="1" <?php if ($is_complete) { echo 'CHECKED'; } ?> <?=$disabled?>/>
                <?php if (!$is_complete && !$is_rejected ) { ?>
                ...
                <?php if (($preauth["status"] == DSS_PREAUTH_PENDING || $preauth["status"] == DSS_PREAUTH_PREAUTH_PENDING) && $_SESSION['admin_access'] == 1) { ?>
                    <a target="_parent" href="manage_vobs.php?delid=<?=$preauth["id"];?>" onclick="return confirm('Do Your Really want to Delete?.');" class="editdel btn btn-danger pull-right" title="DELETE">
                        Delete
                    </a>
                <?php } ?>
            </td>

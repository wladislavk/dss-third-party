...
<script type="text/javascript">
    nv.addGraph(function() {
        ...
        nv.utils.windowResize(function() {
            d3.select('#treatment svg').call(chart);
        });
    ...
    function treatmentCount() {
        ...
        while($r = mysqli_fetch_assoc($q)){ ?>
            activate.push({x: <?= date('U',strtotime($r['activation_date'])); ?>, y: <?= $r['num_activated']; ?>});
            suspend.push({x: <?= date('U',strtotime($r['activation_date'])); ?>, y: <?= $r['num_suspended']; ?>});
            <?php
        } ?>
        return [
            {

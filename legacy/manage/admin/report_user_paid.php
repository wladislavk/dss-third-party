...
<script type="text/javascript">
    ...
    function treatmentCount() {
        ...
        $sql = "select a.Date as paid_date,
            COALESCE( 
                (
                    ...
                    LEFT JOIN (
                        SELECT COALESCE(sum(dl.percase_amount),0) as ledger_amount, pi.docid, pi.adddate 
        ...
        while($r = mysqli_fetch_assoc($q)){ ?>
            paid.push({x: <?= date('U',strtotime($r['paid_date'])); ?>, y: <?= $r['num_paid']; ?>});
            <?php
        } ?>
        return [
            {

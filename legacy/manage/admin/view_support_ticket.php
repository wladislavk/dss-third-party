...
<div id="support_ticket">
    <div class="page-header">
        <h2>
            <?php echo  $t['title']; ?>
            <small>- Category: <?php echo  $t['category']; ?></small>
        </h2>
    </div>
    ...
    <div id="support_responses">
        <?php
        $r_sql = "SELECT r.* FROM dental_support_responses r WHERE ticket_id = '".$db->escape($_REQUEST['ed'])."'";
        ...
        while($r = mysqli_fetch_assoc($r_q)){ ?>
            <div class="panel <?php echo  ($r['response_type']==0)?"panel-info":"panel-success"; ?>">
                ...
                <div class="panel-body">
                    ...
                    if($r['response_type']==0){ ?>
                        | <a href="#" onclick="loadPopup('edit_support_response.php?ed=<?php echo  $_GET['ed']; ?>&id=<?php echo  $r['id']; ?>'); return false;">Edit</a>
                        <?php
                    } ?>
                </div>
            </div>
    ...
    <div id="respond">
        ...
        <form action="<?php echo  $_SERVER['PHP_SELF']; ?>?ed=<?php echo  $_REQUEST['ed']; ?>" method="post" enctype="multipart/form-data">
            ...
            <div style=" width:300px;" class="clearfix">
                <div id="attachments">
                    <span>
                        <input type="file" name="attachment[]" id="attachment1" class="attachment" onchange="$('#add_attachment_but').show()" style="height:auto;width:auto;" />

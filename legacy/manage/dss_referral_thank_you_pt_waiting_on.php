...
$appt_date = date('F d, Y', strtotime($appt_date));
...
?>
<form action="/manage/dss_referral_thank_you_pt_waiting_on.php?pid=<?=$patientid?>&lid=<?=$letterid?><?php print (!empty($_GET['backoffice']) && $_GET['backoffice'] == 1 ? "&backoffice=".$_GET['backoffice'] : ""); ?>" method="post" class="letter">
...
foreach ($letter_contacts as $key => $contact) {
	...
	<div align="right">
		...
		&nbsp;&nbsp;&nbsp;&nbsp;
		<button class="addButton" onclick="Javascript: window.open('dss_referral_thank_you_pt_waiting_on_print.php?pid=<?=$_GET['pid'];?>','Print_letter','width=800,height=500,scrollbars=1');" >
			Print Letter 
		</button>
		&nbsp;&nbsp;&nbsp;&nbsp;
		<button class="addButton" onclick="Javascript: window.open('dss_referral_thank_you_pt_waiting_on_word.php?pid=<?=$_GET['pid'];?>','word_letter','width=800,height=500,scrollbars=1');" >
			Word Document
		</button>
		&nbsp;&nbsp;&nbsp;&nbsp;

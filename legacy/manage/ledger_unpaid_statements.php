...
$find_sql = "SELECT sum(dl.amount) as amount, sum(dl.paid_amount) as paid_amount, p.firstname, p.lastname, p.patientid
...
while ($find_myarray = mysqli_fetch_array($find_my)) {
    ...
    if(round($find_myarray['amount'],2) != round($paid_amount,2)) {
        ...
        $sql = "SELECT dl.amount, sum(pay.amount) as paid_amount
            FROM dental_ledger dl
            LEFT JOIN dental_ledger_payment pay on pay.ledgerid = dl.ledgerid
        ...
        if(mysqli_num_rows($my) == 0) {
            ...
            while($myarray = mysqli_fetch_array($my)) {
                ...
                if($myarray[0] == 'ledger' && !$myarray['primary_claim_id'] && $myarray['status'] == DSS_TRXN_PENDING){
        ...
        <table cellspacing="2">
            ...
            <tr>
                ...
                <td>'.number_format(st($cur_bal),2).'</td>
            </tr>
            <tr>

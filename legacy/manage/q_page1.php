    ...
    if (!$isHistoricView && !empty($_GET['own']) && $_GET['own'] == 1) {
        ...
        if ($_GET['own_completed'] == 1) {
            ...
            if ($db->getNumberRows($q1_sql) == 0) {
                ...
                $qPage1IdRow = $db->getRow($q1_sql);
                $qPage1Id = $qPage1IdRow['q_page1id'];
    ...
    if ($exist_row['symptoms_status'] == 0 && $exist_row['sleep_status'] == 0 && $exist_row['treatments_status'] == 0 && $exist_row['history_status'] == 0) { ?>
        ...
        <form id="q_page1frm"  class="q_form" name="q_page1frm" action="<?php echo $_SERVER['PHP_SELF'];?>?pid=<?php echo $_GET['pid']?><?= $isHistoricView ? "&history_id=$historyId" : '' ?>" method="post">
            ...
            <div style="float:right;">
                ...
                <button class="do-backup hidden" title="Save a copy of the last saved values" onclick="return confirm('Warning! Archiving this page will change the patient\'s baseline sleep symptoms. If you desire to update the patient\'s symptoms then do so in the Summary Tab under Subjective Tab. Do you wish to alter the patient\'s baseline symptoms?')" >
                ...
                <input type="submit" name="q_pagebtn" value="Save" <?= $isHistoricView ? 'disabled' : '' ?> onclick="return confirm('Warning! Saving this page will change the patient\'s baseline sleep symptoms. If you desire to update the patient\'s symptoms then do so in the Summary Tab under Subjective Tab. Do you wish to alter the patient\'s baseline symptoms?')" />
                <input type="submit" name="q_pagebtn_proceed" value="Save And Proceed" <?= $isHistoricView ? 'disabled' : '' ?> onclick="return confirm('Warning! Saving this page will change the patient\'s baseline sleep symptoms. If you desire to update the patient\'s symptoms then do so in the Summary Tab under Subjective Tab. Do you wish to alter the patient\'s baseline symptoms?')" />
                &nbsp;&nbsp;&nbsp;
            </div>
            ...
            <table width="98%" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF" align="center">
                <tr>
                    <td colspan="2">
                        Exam date:
                        <input type="text" id="exam_date" name="exam_date" class="calendar" value="<?= date('m/d/Y H:i', $parsedDate) ?>" />
                        <script type="text/javascript">

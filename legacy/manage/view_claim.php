        ...
        if ($db->query($dsql)) {
            if ($eob['status'] == DSS_CLAIM_DISPUTE) {
                $del_pay = "DELETE FROM dental_ledger_payment 
                    WHERE payer=".DSS_TRXN_PAYER_PRIMARY." 
                    AND ledgerid in (SELECT ledgerid FROM dental_ledger dl WHERE dl.primary_claim_id=".$db->escape($_GET['claimid']).")";
                ...
                $del_pay = "DELETE FROM dental_ledger_payment  
                    WHERE payer=".DSS_TRXN_PAYER_SECONDARY." 
                    AND ledgerid in (SELECT ledgerid FROM dental_ledger dl WHERE dl.primary_claim_id=".$db->escape($_GET['claimid']).")";
        ...
        <?php } else {
            ...
            foreach ($my as $myarray) {
                ...
                <tr class="<?php echo $tr_class;?> <?php echo  $myarray['ledger']; ?>" <?php echo $style; ?>>
                    ...
                    <td <?php if($myarray['ledger']=="eob"){ echo 'onclick="window.location=\'display_file.php?f='.$myarray['filename'].'\'"'; } ?> valign="top">
                        <?php echo (!empty($dss_claim_status_labels[$myarray["status"]]) ? $dss_claim_status_labels[$myarray["status"]] : ''); ?>
                    </td>
                    <td valign="top">

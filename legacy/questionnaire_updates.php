...
if (true) { //to prevent output if set to false
    ...
    *****************************/
    ...
    while ($r = mysqli_fetch_assoc($q)) {
        ...
        if (trim($othercomp) != '') {
            if ($run_updates) {
                mysqli_query($con, "UPDATE `dental_q_page1` SET complaintid=CONCAT(complaintid,'0|1~') WHERE q_page1id = (SELECT max_id FROM (SELECT MAX(q_page1id) AS max_id FROM `dental_q_page1` WHERE patientid=".$pid.") AS t)");
            }
        }
        $upsql = "UPDATE `dental_q_page1` SET other_complaint='".$othercomp."' WHERE q_page1id = (SELECT max_id FROM (SELECT MAX(q_page1id) AS max_id FROM `dental_q_page1` WHERE patientid=".$pid.") AS t)";
        if ($run_updates) {

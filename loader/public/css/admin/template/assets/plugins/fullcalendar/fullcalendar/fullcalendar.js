...
function Header(calendar, options) {
	...
	function renderSection(position) {
		...
		if (buttonStr) {
			$.each(buttonStr.explode(' '), function(i) {
				if (i > 0) {
					e.append("<span class='fc-header-space'/>");
				}
				var prevButton;
				$.each(this.explode(','), function(j, buttonName) {
					if (buttonName == 'title') {
	...
	function normalizeEvent(event) {
		...
		if (event.className) {
			if (typeof event.className == 'string') {
				event.className = event.className.explode(/\s+/);
			}
		}else{
			event.className = [];
		}
	...
	function normalizeSource(source) {
		if (source.className) {
			...
			if (typeof source.className == 'string') {
				source.className = source.className.explode(/\s+/);
			}
		}else{
			source.className = [];
		}

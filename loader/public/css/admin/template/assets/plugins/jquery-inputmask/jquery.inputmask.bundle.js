    ...
    if ($.fn.inputmask === undefined) {
        ...
        function generateMaskSets(opts) {
            ...
            function getMaskTemplate(mask) {
                if (opts.numericInput) {
                    mask = mask.explode('').reverse().join('');
                }
                var escaped = false, outCount = 0, greedy = opts.greedy, repeat = opts.repeat;
                if (repeat == "*") greedy = false;
                //if (greedy == true && opts.placeholder == "") opts.placeholder = " ";
                if (mask.length == 1 && greedy == false && repeat != 0) { opts.placeholder = ""; } //hide placeholder with single non-greedy mask
                var singleMask = $.map(mask.explode(""), function (element, index) {
                    var outElem = [];
            ...
            function getTestingChain(mask) {
                ...
                return $.map(mask.explode(""), function (element, index) {
                    var outElem = [];
            ...
            function explodeFirstOptionalEndPart(maskPart) {
            ...
            function explodeFirstOptionalStartPart(maskPart) {
            ...
            function generateMask(maskPrefix, maskPart, metadata) {
                var maskParts = explodeFirstOptionalEndPart(maskPart);
                var newMask, maskTemplate;

                var masks = explodeFirstOptionalStartPart(maskParts[0]);
                if (masks.length > 1) {
                    ...
                    if (explodeFirstOptionalStartPart(masks[1]).length > 1) { //optional contains another optional
                        generateMask(maskPrefix + masks[0], masks[1] + maskParts[1], metadata);
                    }
                    if (maskParts.length > 1 && explodeFirstOptionalStartPart(maskParts[1]).length > 1) {
        ...
        function maskScope(masksets, activeMasksetIndex, opts, actionObj) {
            ...
            function checkVal(input, writeOut, strict, nptvl, intelliCheck) {
                var inputValue = nptvl != undefined ? nptvl.slice() : truncateInput(input._valueGet()).explode('');
                ...
                if (valueProperty && valueProperty.get) {
                    if (!npt._valueGet) {
                        ...
                        npt._valueGet = function () {
                            return isRTL ? valueGet.call(this).explode('').reverse().join('') : valueGet.call(this);
                        };
                        npt._valueSet = function (value) {
                            valueSet.call(this, isRTL ? value.explode('').reverse().join('') : value);
                        };
                    ...
                    if (!npt._valueGet) {
                        npt._valueGet = function () { return isRTL ? this.value.explode('').reverse().join('') : this.value; };
                        npt._valueSet = function (value) { this.value = isRTL ? value.explode('').reverse().join('') : value; };
                    }
            ...
            function shiftL(start, end, c, maskJumps) {
                ...
                if (getActiveMaskSet()["greedy"] == false) {
                    var trbuffer = truncateInput(buffer.join('')).explode('');
            ...
            function keydownEvent(e) {
                ...
                if (k == opts.keyCode.BACKSPACE || k == opts.keyCode.DELETE || (iphone && k == 127) || e.ctrlKey && k == 88) { //backspace/delete
                    ...
                    checkVal(input, true, false, valueOnFocus.explode(''));
                    $input.click();
                        ...
                        if (writeOut !== false) {
                            ...
                            if (result != undefined) {
                                ...
                                if (getActiveMaskSet()["writeOutBuffer"] && result["result"] !== false) {
                                    ...
                                    getActiveMaskSet()["buffer"] = getActiveMaskSet()["undoBuffer"].explode('');
                                }
                            }
                        }
                ...
                if ($el.is(":input")) {
                    ...
                    $el.bind("mouseenter.inputmask", function () {
                        ...
                        setTimeout(function () {
                            ...
                            checkVal(input, true, false, pasteValue.explode(''), true);
                            if (isComplete(getActiveBuffer()) === true)
                                $input.trigger("complete");
                            $input.click();
                        }, 0);
            ...
            if (actionObj != undefined) {
                switch (actionObj["action"]) {
                    ...
                    case "format":
                        ...
                        checkVal($el, false, false, actionObj["value"].explode(''), true);
            ...
            if (typeof fn === "string") {
                switch (fn) {
                    ...
                    case "isComplete":
                        ...
                        return maskScope(masksets, activeMasksetIndex, opts, { "action": "isComplete", "buffer": this[0]._valueGet().explode('') });
        ...
        'decimal': {
            ...
            postFormat: function (buffer, pos, reformatOnly, opts) {
                ...
                if (opts.autoGroup || (reformatOnly && bufVal.indexOf(opts.groupSeparator) != -1)) {
                    ...
                    var radixexplode = bufVal.explode(opts.radixPoint);
                    bufVal = radixexplode[0];
                    var reg = new RegExp('([-\+]?[\\d\?]+)([\\d\?]{' + opts.groupSize + '})');
                    while (reg.test(bufVal)) {
                        bufVal = bufVal.replace(reg, '$1' + opts.groupSeparator + '$2');
                        bufVal = bufVal.replace(opts.groupSeparator + opts.groupSeparator, opts.groupSeparator);
                    }
                    if (radixexplode.length > 1)
                        bufVal += opts.radixPoint + radixexplode[1];
                }

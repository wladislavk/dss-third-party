    ...
    if($let_r['use_letters'] && $let_r['intro_letters']){
        ...
        foreach ($mdcontacts as $contact) {
            if ($contact != "Not Set") {
                ...
                $letter_result = $db->query($letter_query);
...
if($_POST["patientsub"] == 1){
    ...
    if($_POST["ed"] != ""){ //existing patient (update) 
        $s_sql = "SELECT referred_by, referred_source, email, password, registration_status, p_m_relation,
                      		p_m_partyfname, p_m_partylname, ins_dob, p_m_ins_type, p_m_ins_ass, p_m_ins_co, 
...
$vob_my = $db->getResults($vob_sql);
$vob_myarray = $vob_my[0];
$pending_vob = count($vob_my);
...
if($themyarray['registration_status']==1){
    echo "PIN Code: " .  $themyarray['access_code'];
} ?>
                            </div>
                        </div>
            	    </li>
...
<script src="js/add_patient.js" type="text/javascript"></script>

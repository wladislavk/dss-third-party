...
if($_POST["doc_educationalub"] == 1)
{
	...
	if($_POST["ed"] != "")
	{
		$ed_sql = "update dental_doc_educational set title = '".s_for($_POST["title"])."', docid = '".s_for($doc_id)."', description = '".s_for($_POST["description"])."', video_file = '".s_for($banner1)."', doc_file = '".s_for($banner2)."', sortby = '".s_for($sby)."', status = '".s_for($_POST["status"])."' where doc_educationalid='".$_POST["ed"]."'";
		...
		<script type="text/javascript">
			//alert("<?=$msg;?>");
			parent.window.location='manage_doc_educational.php?msg=<?=$msg;?>';
		</script>
		<?
	...
	{
		$ins_sql = "insert into dental_doc_educational set title = '".s_for($_POST["title"])."', docid = '".s_for($doc_id)."', description = '".s_for($_POST["description"])."', video_file = '".s_for($banner1)."', doc_file = '".s_for($banner2)."', sortby = '".s_for($sby)."', status = '".s_for($_POST["status"])."', adddate=now(),ip_address='".$_SERVER['REMOTE_ADDR']."'";
...
<?php require_once dirname(__FILE__) . '/includes/popup_top.htm'; ?>
    ...
    <?
    $thesql = "select * from dental_doc_educational where doc_educationalid='".$_REQUEST["ed"]."'";
	...
	if($themyarray["doc_educationalid"] != '')
	{
		$but_text = "Edit ";
	}
	else
    ...
    <? }?>
    <form name="doc_educationalfrm" action="<?=$_SERVER['PHP_SELF'];?>?add=1" method="post" onSubmit="return doc_educationalabc(this)" enctype="multipart/form-data">
    <table class="table table-bordered table-hover">
        <tr>
            <td colspan="2" class="cat_head">
               <?=$but_text?> Educational Materials
               <? if($title <> "") {?>
               		&quot;<?=$title;?>&quot;
               <? }?>
            </td>
        ...
        <tr>
            <td  colspan="2" align="center">
                ...
                <input type="hidden" name="doc_educationalub" value="1" />
                <input type="hidden" name="ed" value="<?=$themyarray["doc_educationalid"]?>" />
                <input type="submit" value="<?=$but_text?> Educational Materials " class="btn btn-primary">
            </td>
        </tr>
    </table>
    </form>

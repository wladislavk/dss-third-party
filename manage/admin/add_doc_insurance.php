...
if($_POST["doc_insuranceub"] == 1)
{
	...
	if($_POST["ed"] != "")
	{
		...
		<script type="text/javascript">
			//alert("<?=$msg;?>");
			parent.window.location='manage_doc_insurance.php?msg=<?=$msg;?>';
		</script>
		<?
		die();
	}
...
<?php require_once dirname(__FILE__) . '/includes/popup_top.htm'; ?>
    ...
    <?
    $thesql = "select * from dental_doc_insurance where doc_insuranceid='".$_REQUEST["ed"]."'";
    ...
    <? }?>
    <form name="doc_insurancefrm" action="<?=$_SERVER['PHP_SELF'];?>?add=1" method="post" onSubmit="return doc_insuranceabc(this)" enctype="multipart/form-data">
    <table class="table table-bordered table-hover">
        <tr>
            <td colspan="2" class="cat_head">
               <?=$but_text?> Insurance Information
               <? if($title <> "") {?>
               		&quot;<?=$title;?>&quot;
               <? }?>
            </td>
        ...
        <tr>
            <td  colspan="2" align="center">
                ...
                <input type="submit" value="<?=$but_text?> Insurance Information " class="btn btn-primary">
		<?php if($themyarray["doc_insuranceid"] != '' && $_SESSION['admin_access']==1){ ?>
                    <a href="manage_doc_insurance.php?delid=<?=$themyarray["doc_insuranceid"];?>" onclick="javascript: return confirm('Do Your Really want to Delete?.');" target="_parent" class="editdel btn btn-danger pull-right" title="DELETE">
                                                Delete
                                        </a>
		<?php } ?>
            </td>

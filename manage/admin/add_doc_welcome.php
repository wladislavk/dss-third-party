...
if($_POST["doc_welcomeub"] == 1)
{
	...
	if($_POST["ed"] != "")
	{
		...
		<script type="text/javascript">
			//alert("<?=$msg;?>");
			parent.window.location='manage_doc_welcome.php?msg=<?=$msg;?>';
		</script>
		<?
		die();
	}
...
<?php require_once dirname(__FILE__) . '/includes/popup_top.htm'; ?>
    ...
    <?
    $thesql = "select * from dental_doc_welcome where doc_welcomeid='".$_REQUEST["ed"]."'";
    ...
    <? }?>
    <form name="doc_welcomefrm" action="<?=$_SERVER['PHP_SELF'];?>?add=1" method="post" onSubmit="return doc_welcomeabc(this)" enctype="multipart/form-data">
    <table class="table table-bordered table-hover">
        <tr>
            <td colspan="2" class="cat_head">
               <?=$but_text?> Welcome Information
               <? if($title <> "") {?>
               		&quot;<?=$title;?>&quot;
               <? }?>
            </td>
        ...
        <tr>
            <td  colspan="2" align="center">
                ...
                <input type="submit" value="<?=$but_text?> Welcome Information " class="btn btn-primary">
		<?php if($themyarray["doc_welcomeid"] != '' && $_SESSION['admin_access']==1){ ?>
                    <a href="manage_doc_welcome.php?delid=<?=$themyarray["doc_welcomeid"];?>" onclick="javascript: return confirm('Do Your Really want to Delete?.');" target="_parent" class="editdel btn btn-danger pull-right" title="DELETE">
                                                Delete
                                        </a>
		<?php } ?>
            </td>

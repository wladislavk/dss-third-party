...
if(!empty($_POST["doc_welcomeub"]) && $_POST["doc_welcomeub"] == 1)
{
	...
	if($_POST["ed"] != "")
	{
		$ed_sql = "update dental_doc_welcome set title = '".s_for($_POST["title"])."', docid = '".s_for($doc_id)."', description = '".s_for($_POST["description"])."', video_file = '".s_for($banner1)."', doc_file = '".s_for($banner2)."', sortby = '".s_for($sby)."', status = '".s_for($_POST["status"])."' where doc_welcomeid='".$_POST["ed"]."'";
	...
	{
		$ins_sql = "insert into dental_doc_welcome set title = '".s_for($_POST["title"])."', docid = '".s_for($doc_id)."', description = '".s_for($_POST["description"])."', video_file = '".s_for($banner1)."', doc_file = '".s_for($banner2)."', sortby = '".s_for($sby)."', status = '".s_for($_POST["status"])."', adddate=now(),ip_address='".$_SERVER['REMOTE_ADDR']."'";
...
<?php include_once dirname(__FILE__) . '/includes/popup_top.htm'; ?>
    ...
    <?php }?>
    <form name="doc_welcomefrm" action="<?php echo $_SERVER['PHP_SELF'];?>?add=1" method="post" onSubmit="return doc_welcomeabc(this)" enctype="multipart/form-data">
    <table class="table table-bordered table-hover">
        <tr>
            <td colspan="2" class="cat_head">
               <?php echo $but_text?> Welcome Information
               <?php if($title <> "") {?>
               		&quot;<?php echo $title;?>&quot;
               <?php }?>
            </td>
        ...
        <tr>
            <td  colspan="2" align="center">
                ...
                <input type="hidden" name="ed" value="<?php echo $themyarray["doc_welcomeid"]?>" />
                <input type="submit" value="<?php echo $but_text?> Welcome Information " class="btn btn-primary">
		<?php if($themyarray["doc_welcomeid"] != '' && $_SESSION['admin_access']==1){ ?>
                    <a href="manage_doc_welcome.php?delid=<?php echo $themyarray["doc_welcomeid"];?>" onclick="javascript: return confirm('Do Your Really want to Delete?.');" target="_parent" class="editdel btn btn-danger pull-right" title="DELETE">
                                                Delete
                                        </a>
		<?php } ?>
            </td>

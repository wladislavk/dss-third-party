...
if($_POST["mult_exam_teethsub"] == 1)
{
	...
	<script type="text/javascript">
		//alert("<?=$msg;?>");
		parent.window.location='manage_exam_teeth.php?msg=<?=$msg;?>';
	</script>
	<?
	die();
}

if($_POST["exam_teethsub"] == 1)
{
...
<?php require_once dirname(__FILE__) . '/includes/popup_top.htm'; ?>
    ...
    <table class="table table-bordered table-hover">
		...
		<?php if($themyarray["exam_teethid"] != '' && $_SESSION['admin_access']==1){ ?>
                    <a href="manage_exam_teeth.php?delid=<?=$themyarray["exam_teethid"];?>" onclick="javascript: return confirm('Do Your Really want to Delete?.');" target="_parent" class="editdel btn btn-danger pull-right" title="DELETE">
                                                Delete
                                        </a>
		<?php } ?>
            </td>

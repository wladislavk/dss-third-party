...
if(!empty($_POST["mult_ins_typesub"]) && $_POST["mult_ins_typesub"] == 1)
{
	$op_arr = explode("\n",trim($_POST['ins_type']));
...
<?php include_once dirname(__FILE__) . '/includes/popup_top.htm'; ?>
    ...
    <?
    $thesql = "select * from dental_ins_type where ins_typeid='".(!empty($_REQUEST["ed"]) ? $_REQUEST["ed"] : '')."'";
    ...
    <?php }?>
    <form name="ins_typefrm" action="<?php echo $_SERVER['PHP_SELF'];?>?add=1" method="post" onSubmit="return ins_typeabc(this)">
    <table class="table table-bordered table-hover">
        ...
        <tr bgcolor="#FFFFFF">
            ...
            <td valign="top" class="frmdata">
                <input type="text" name="ins_type" value="<?php echo $ins_type?>" class="form-control" /> 
                <span class="red">*</span>				
            </td>
        </tr>
        <tr bgcolor="#FFFFFF">
        ...
        <tr>
            <td  colspan="2" align="center">
                ...
                <input type="hidden" name="ed" value="<?php echo $themyarray["ins_typeid"]?>" />
                <input type="submit" value="<?php echo $but_text?> Insurance Type" class="btn btn-primary">
		<?php if($themyarray["ins_typeid"] != '' && $_SESSION['admin_access']==1){ ?>
                    <a href="manage_ins_type.php?delid=<?php echo $themyarray["ins_typeid"];?>" onclick="javascript: return confirm('Do Your Really want to Delete?.');" target="_parent" class="editdel btn btn-danger pull-right" title="DELETE">
                                                Delete
                                        </a>
		<?php } ?>
            </td>

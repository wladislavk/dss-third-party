...
$paymentsAdded = insertLedgerPayments(
    $claimId, $_POST['payments'], $_POST['payment_type'], $_POST['payer'], 0, $_SESSION['adminid']
);
...
if ($newStatus !== false) {
    ...
    claim_status_history_update($claimId, $newStatus, $claimData['status'], 0, $_SESSION['adminid']);
}

if ($secondaryStatus !== false && empty($_POST['empty-claim'])) {
    $msg = 'Payment Successfully Added\n\nPrimary Insurance claim closed. This patient has secondary insurance and a claim has been auto-generated for the Secondary Insurer.';

    ClaimFormData::createSecondaryClaim($patientId, 0, $claimId, $secondaryStatus);
}
...
if (empty($paymentId) && empty($_POST['empty-claim'])) {
    ...
    claim_history_update($claimId, 0, $_SESSION['adminuserid']);
}

		...
		?>
			<tr class="<?=$tr_class;?>">
				...
				<td valign="top" align="center">
					<?php if(is_super($_SESSION['admin_access'])){ ?>
					<input type="text" name="sortby_<?=$myarray['deviceid'];?>" value="<?=st($myarray['sortby'])?>" class="form-control text-center" style="width:5em"/>
					<?php }else{ ?>
						<?= $myarray['sortby']; ?>
					<?php } ?>
				</td>	
				...
				<td valign="top">
					<?php if(is_super($_SESSION['admin_access'])){ ?>
					<a href="Javascript:;"  onclick="Javascript: loadPopup('add_device.php?ed=<?=$myarray["deviceid"];?>');" title="Edit" class="btn btn-primary btn-sm">
						Edit

...
{
	$del_sql = "delete from dental_evaluation_new where evaluation_newid='".$_REQUEST["delid"]."'";
...
$sql = "select * from dental_evaluation_new order by sortby";
...
if(!empty($_POST['sortsub']) && $_POST['sortsub'] == 1)
{
	foreach($_POST['sortby'] as $val)
	{
		...
		$up_sort_sql = "update dental_evaluation_new set sortby='".s_for($val)."' where evaluation_newid='".$smyarray["evaluation_newid"]."'";
...
<div align="right">
	<button onclick="Javascript: loadPopup('add_evaluation_new.php');" class="btn btn-success">
		Add New Evaluation New
	...
	else
	{
		...
		?>
			<tr class="<?php echo $tr_class;?>">
				<td valign="top">
					<?php echo st($myarray["evaluation_new"]);?>
				</td>
				...
				<td valign="top">
					<?php if(is_super($_SESSION['admin_access'])){ ?>
					<a href="Javascript:;"  onclick="Javascript: loadPopup('add_evaluation_new.php?ed=<?php echo $myarray["evaluation_newid"];?>');" title="Edit" class="btn btn-primary btn-sm">
						Edit

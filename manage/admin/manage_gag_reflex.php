...
if($_REQUEST["delid"] != "")
{
	$del_sql = "delete from dental_gag_reflex where gag_reflexid='".$_REQUEST["delid"]."'";
	mysql_query($del_sql);
...
$sql = "select * from dental_gag_reflex order by sortby";
$my = mysql_query($sql);
...
if($_POST['sortsub'] == 1)
{
	foreach($_POST['sortby'] as $val)
	{
		...
		$up_sort_sql = "update dental_gag_reflex set sortby='".s_for($val)."' where gag_reflexid='".$smyarray["gag_reflexid"]."'";
	...
	else
	{
		...
		?>
			<tr class="<?=$tr_class;?>">
				...
				<td valign="top">
					<a href="Javascript:;"  onclick="Javascript: loadPopup('add_gag_reflex.php?ed=<?=$myarray["gag_reflexid"];?>');" class="editlink" title="EDIT">
						Edit
					</a>
                    
                    <a href="<?=$_SERVER['PHP_SELF']?>?delid=<?=$myarray["gag_reflexid"];?>" onclick="javascript: return confirm('Do Your Really want to Delete?.');" class="dellink" title="DELETE">
						Delete
					</a>
				</td>
			</tr>

...
foreach ($dental_letters as $key => $letter) {
  ...
  if ($status == "pending") {
		...
		if ($age_in_days > 7) $dental_letters[$key]['delivery_status'] = "red";
    if ($age_in_days < 7) $dental_letters[$key]['delivery_status'] = "yellow";
  } elseif ($status == "sent") {
		$dental_letters[$key]['delivery_status'] = "green";
  }
}
  ...
  while ($i < count($dental_letters) && $i < $end) {
    ...
    if ($dental_letters[$i]['old']) {
      ...
      $alert = null;
		...
		$priority = $dental_letters[$key]['delivery_status'];
    
    print "<tr$alert><td>$franchisee</td><td>$username</td><td>$name</td><td><a href=\"$url\">$subject</a></td><td>$sentto</td><td>$generated</td>".($status == "pending" ? "<td class=\"$priority\">$sent</td>" : "").($status == "sent" ? "<td class=\"$priority\">$delivered</td>" : "")."</tr>";
    $i++;
  }
?>
</table>

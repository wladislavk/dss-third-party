...
if($_POST['sortsub'] == 1)
{
	foreach($_POST['sortby'] as $val)
	{
		...
		$up_sort_sql = "update dental_nasal_passages set sortby='".s_for($val)."' where nasal_passagesid='".$smyarray["nasal_passagesid"]."'";
	...
	else
	{
		...
		?>
			<tr class="<?=$tr_class;?>">
				...
				<td valign="top">
					<a href="Javascript:;"  onclick="Javascript: loadPopup('add_nasal_passages.php?ed=<?=$myarray["nasal_passagesid"];?>');" class="editlink" title="EDIT">
						Edit
					</a>
                    
                    <a href="<?=$_SERVER['PHP_SELF']?>?delid=<?=$myarray["nasal_passagesid"];?>" onclick="javascript: return confirm('Do Your Really want to Delete?.');" class="dellink" title="DELETE">
						Delete
					</a>
				</td>
			</tr>

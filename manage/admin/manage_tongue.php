...
if($_REQUEST["delid"] != "")
{
	$del_sql = "delete from dental_tongue where tongueid='".$_REQUEST["delid"]."'";
	mysql_query($del_sql);
...
$sql = "select * from dental_tongue order by sortby";
$my = mysql_query($sql);
...
if($_POST['sortsub'] == 1)
{
	foreach($_POST['sortby'] as $val)
	{
		...
		$up_sort_sql = "update dental_tongue set sortby='".s_for($val)."' where tongueid='".$smyarray["tongueid"]."'";
	...
	else
	{
		...
		?>
			<tr class="<?=$tr_class;?>">
				...
				<td valign="top">
					<a href="Javascript:;"  onclick="Javascript: loadPopup('add_tongue.php?ed=<?=$myarray["tongueid"];?>');" class="editlink" title="EDIT">
						Edit
					</a>
                    
                    <a href="<?=$_SERVER['PHP_SELF']?>?delid=<?=$myarray["tongueid"];?>" onclick="javascript: return confirm('Do Your Really want to Delete?.');" class="dellink" title="DELETE">
						Delete
					</a>
				</td>
			</tr>

...
<?php
  ...
  $sql = "SELECT n.*, CASE WHEN n.create_type='0' THEN CONCAT(a.first_name, ' ', a.last_name)
          ...
          left join admin a ON n.creator_id = a.adminid where n.claim_id='".mysql_real_escape_string($_GET['claimid'])."'";
...
<div class="fullwidth">
  ...
  <a href="#" onclick="loadPopup('add_claim_note.php?claim_id=<?php echo $_GET['claimid'];?>&pid=<?php echo $_GET['pid'];?>');return false;" class="button" style="float:right;">Add Note</a>
  <br /><br />
  ...
  <?php if ($my) foreach ($my as $r) { ?>
          <div class="response_type_<?php echo  $r['create_type']; ?>" >
            ...
            <?php if($r['create_type'] == '1' && $r['creator_id'] == $_SESSION['userid']) { ?>
              <br />
              <a href="#" onclick="loadPopup('add_claim_note.php?claim_id=<?php echo $_GET['claimid'];?>&pid=<?php echo $_GET['pid'];?>&nid=<?php echo $r['id'];?>');return false;" class="button">Edit Note</a>
      	    <?php } ?>
          </div>
  <?php } ?>

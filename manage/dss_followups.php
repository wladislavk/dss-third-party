...
<div style="width:<?php echo $bodywidth; ?>px;">
    ...
    if ($fuquery_array) {
        ...
        foreach ($fuquery_array as $fuquery) {
            ...
            if ($numrows) { ?>
                <form style="float:left;" id="sleepstdyupdate_<?= $fuquery['followupid'];?>" class="sleepstudyupdate" method="post" enctype="multipart/form-data" action="<?= $_SERVER['PHP_SELF']."&pid=".(!empty($_GET['pid']) ? $_GET['pid'] : ''); ?>">
                    <input type="hidden" name="id" value="<?= $fuquery['followupid'];?>" />
                    <table id="sleepstudyscrolltable" style="padding:0;margin-top:-3px">
                        <tr style="background: #444;height: 30px;">
                            <td colspan="4" style="background: #444;">
                                <span style="color: #ccc;"><?= $fuquery['followupid'];?></span>
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #F9FFDF;">
                                <input type="text" size="12" style="width:75px;"  class="calendar" id="ep_dateadd_<?= $fuquery['followupid'];?>" name="ep_dateadd" value="<?= ($fuquery['ep_dateadd'])?date('m/d/Y', strtotime($fuquery['ep_dateadd'])):''; ?>" />
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #E4FFCF;">
                        ...
                        <tr>
                            <td style="background: #F9FFDF;">
                                <input type="text" size="12" name="dsetadd" value="<?= $fuquery['dsetadd']; ?>" />
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #E4FFCF;">
                                <select name="nightsperweek" style="width:150px;">
                                    <?php
                                    for ($i = 0; $i <= 7; $i++) { ?>
                                        <option <?= ($i == $fuquery['nightsperweek']) ? 'selected' : '' ?> value="<?= $i ?>"><?= $i ?></option>
                                        <?php
                                    } ?>
                                </select>
                            </td>
                        ...
                        <tr>
                            <td style="background: #E4FFCF;">
                                <input type="text" size="12" id="ep_eadd_<?= $fuquery['followupid']; ?>" name="ep_eadd" value="<?= $fuquery['ep_eadd'];?>" onclick="loadPopup('summ_subj_ess.php?pid=<?= (!empty($_GET['pid']) ? $_GET['pid'] : ''); ?>&id=<?= $fuquery['followupid']; ?>');return false;" />
                            </td>
                        </tr>
                        <!--- ESS ANSWERS -->
                        <tr style="display:none;">
                            <td style="background: #E4FFCF;">
                                <?php
                                $epworth_sql = "
                                    select e.*, fu.answer from dental_epworth e 
                                ...
                                foreach ($epworth_my as $epworth_myarray) { ?>
                                    <input type="text" size="12" id="epworth_<?= $fuquery['followupid']; ?>_<?= $epworth_myarray['epworthid']; ?>" name="epworth_<?= $fuquery['followupid']; ?>_<?= $epworth_myarray['epworthid']; ?>" value="<?= $epworth_myarray['answer']; ?>" /><br />
                                    <?php
                                } ?>
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #F9FFDF;">
                                <input type="text" size="12" id="ep_tsadd_<?= $fuquery['followupid']; ?>" name="ep_tsadd" value="<?= $fuquery['ep_tsadd']; ?>" onclick="loadPopup('summ_subj_tss.php?pid=<?= (!empty($_GET['pid']) ? $_GET['pid'] : ''); ?>&id=<?= $fuquery['followupid']; ?>');return false;" />
                            </td>
                        </tr>
                        <tr style="display:none;">
                            <td style="background: #E4FFCF;">
                                ...
                                for ($thorntonid = 1; $thorntonid <= 5; $thorntonid++) {
                                    $t_sql = "
                                        SELECT answer FROM dentalsummfu_tss
                                        WHERE followupid='".$db->escape($fuquery['followupid'])."' 
                                        AND thorntonid='".$thorntonid."'
                                    ";
                                    $thornton_myarray = $db->getRow($t_sql); ?>
                                    <input type="text" size="12" id="thornton_<?= $fuquery['followupid']; ?>_<?= $thorntonid; ?>" name="thornton_<?= $fuquery['followupid'];?>_<?= $thorntonid; ?>" value="<?= $thornton_myarray['answer']; ?>" /><br />
                                    <?php
                                } ?>
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #E4FFCF;">
                                <input type="text" size="12" name="ep_sadd" value="<?= $fuquery['ep_sadd']; ?>" />
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #E4FFCF;">
                                <input type="text" size="12" name="ep_eladd" value="<?= $fuquery['ep_eladd']; ?>" />
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #F9FFDF;">
                                <input type="text" size="12" name="sleep_qualadd" value="<?= $fuquery['sleep_qualadd']; ?>" />
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #E4FFCF;">
                        ...
                        <tr>
                            <td style="background: #F9FFDF;">
                                <input type="text" size="12" name="ep_wadd" value="<?= $fuquery['ep_wadd']; ?>" />
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #E4FFCF;">
                                <input type="text" size="12" style="width:90px;" name="wapnadd" value="<?= $fuquery['wapnadd']; ?>" />
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #F9FFDF;">
                                <input type="text" size="12" name="hours_sleepadd" value="<?= $fuquery['hours_sleepadd'];?>" />
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #E4FFCF;">
                                <input type="text" size="12" name="appt_notesadd" value="<?= $fuquery['appt_notesadd']; ?>" style="width:100px;" />
                            </td>
                        </tr>
                        <tr>
                            <td style="background: #E4FFCF;">
                                <input type="hidden" name="patientid" value="<?= (!empty($_GET['pid']) ? $_GET['pid'] : ''); ?>" />
                                <input type="submit" name="submitupdatefu" onclick="window.onbeforeunload=false;" value="Save Follow Up" id="submitupdatefu_<?= $fuquery['followupid'];?>" style="width:120px;" />

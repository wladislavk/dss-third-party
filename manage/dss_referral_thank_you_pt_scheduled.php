...
/*
$pat_sql = "select * from dental_patients where patientid='".s_for($_GET['pd'])."'";
...
if ($othermd_result) {
	...
	if ($othermd_result['md_referral_list'] != null) {
		$md_referral_array = array_merge($md_referral_array, explode(",", $othermd_result['md_referral_list']));
	}
}
...
?>
<form action="/manage/dss_referral_thank_you_pt_scheduled.php?pid=<?=$patientid?>&lid=<?=$letterid?><?php print ($_GET['backoffice'] == 1 ? "&backoffice=".$_GET['backoffice'] : ""); ?>" method="post" class="letter">

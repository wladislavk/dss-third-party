...
<div align="right">
	<button class="addButton" onclick="Javascript: window.open('dss_referral_thank_you_pt_waiting_on_print.php?fid=<?=$_GET['fid'];?>&pid=<?=$_GET['pid'];?>','Print_letter','width=800,height=500,scrollbars=1');" >
		Print Letter 
	</button>
	&nbsp;&nbsp;&nbsp;&nbsp;
	<button class="addButton" onclick="Javascript: window.open('dss_referral_thank_you_pt_waiting_on_word.php?fid=<?=$_GET['fid'];?>&pid=<?=$_GET['pid'];?>','word_letter','width=800,height=500,scrollbars=1');" >
		Word Document
	</button>
	&nbsp;&nbsp;&nbsp;&nbsp;
</div>
...
Thank you for referring <strong><?=$name;?></strong> to our office. As you recall, <strong><?=$name1?></strong> is a <strong><?=$age;?></strong> year old <strong><?=$pat_myarray['gender']?></strong> with a prior medical history that includes <strong><?=$history_disp;?></strong>.  <strong><?=$h_h;?></strong> medications include <strong><?=$medications_disp?></strong>.  <strong><?=$name1?></strong> had a <strong>sleep test <?=$type_study;?></strong> done at the <strong><?=$sleeplab_name?></strong> on <strong><?=date('F d, Y',strtotime($sleep_study_on))?></strong> which showed an AHI of <strong><?=$ahi?></strong> <? if($rdi <> '') {?>, RDI of <strong><?=$rdi?></strong> <? }?> and low O2 of <strong><?=$sti_o2_1;?></strong>; <strong><?=$s_h;?></strong> was diagnosed with <strong><?=$confirmed_diagnosis;?> <?=$custom_diagnosis;?></strong>.  You referred <strong><?=$h_h1;?></strong> to me for treatment with a dental sleep device.<br><br>

Oral evaluation of <strong><?=$name1?></strong> revealed no contraindications to wearing a dental sleep device. <?=$h_h;?> mandibular range of motion is good at <strong><?=$protrusion_equal?></strong> mm protrusive ROM.   Her TMJ is <strong>???</strong>. <br><br>

Overall, I do believe <strong><?=$name1?></strong> is a good candidate for Dental Device Therapy. However, <strong><?=$s_h;?></strong> is waiting to begin treatment due to <strong>???</strong>.  <br><br>

Thank you again for your confidence and the referral.  We will keep you updated on <?=$h_h?> treatment as it progresses.  <br /><br /> 

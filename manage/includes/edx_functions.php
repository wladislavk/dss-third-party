...
function edx_user_update($id){
  ...
  if($edx_id == '' || $edx_id == '0'){
    ...
    $edx_id = shell_exec('sh ../edx_scripts/edxNewUser.sh '. $username .' '.$r['email'].' "'.$pass.'" "'.$name.'"');
    $u_sql = "UPDATE dental_users SET edx_id='".mysql_real_escape_string($edx_id)."' WHERE userid='".mysql_real_escape_string($id)."'";
    mysql_query($u_sql);
  }else{
    $name = $r['first_name']. ' '.$r['last_name'];
    $pass = sha1($r['username'].'ed&$s8e'.$r['email'].rand());
    $edx_id = shell_exec('sh ../edx_scripts/edxEditUser.sh '.$edx_id.' "'.$r['username'].'" '.$r['email'].' '.$pass.' "'.$name.'"');
  }
...
function edx_user_delete($edx_id){
  if($edx_id != '' && $edx_id != '0'){
    $ed_id = shell_exec('sh ../edx_scripts/edxDeleteUser.sh '.$edx_id);
  }
}

...
$sql = "
    ...
    WHERE (
        (
            (lastname LIKE '" . $names[0] . "%' OR firstname LIKE '" . $names[0] . "%') 
            AND (
                lastname LIKE '" . (!empty($names[1]) ? $names[1] : '') . "%' 
                OR firstname LIKE '" . (!empty($names[1]) ? $names[1] : '') . "%'
            )
        ) OR (
            firstname LIKE '" . $names[0] ."%' 
            AND middlename LIKE '" .(!empty($names[1]) ? $names[1] : '')."%' 
            AND lastname LIKE '" . (!empty($names[2]) ? $names[2] : '') . "%'
        )
    )
    AND p.status=1

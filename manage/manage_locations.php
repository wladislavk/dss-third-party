	...
	else
	{
		...
		?>
			<tr class="<?=$tr_class;?>">
				...
				<td valign="top">
					<a href="Javascript:;"  onclick="Javascript: loadPopup('add_location.php?ed=<?=$myarray["id"];?>');" class="editlink" title="EDIT">
						Edit
					</a>
					|
					<?php if($myarray['default_location']==1){ ?>
						Default Location
					<?php }else{ ?>
					   <a href="manage_locations.php?did=<?= $myarray['id']; ?>"  class="editlink" title="MAKE DEFAULT">
                                                Make Default 
                                          </a>
                    			<?php } ?>
				</td>

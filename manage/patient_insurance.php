...
if(isset($_REQUEST['pisort'])){
  switch($_REQUEST['pisort']){
    case 'address':
	$pisort = "pc.address1";
	break;
    case 'company':
...
<table width="98%" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF" align="center" >
	...
	<? }?>
	<tr class="tr_bg_h">
		<td valign="top" class="col_head  <?= ($_REQUEST['sort'] == 'company')?'arrow_'.strtolower($_REQUEST['sortdir']):''; ?>" width="20%">
			...
			<tr class="<?=$tr_class;?> <?= ($myarray['viewed'])?'':'unviewed'; ?>">
				...
				<td valign="top">
					...
					<span class="phonemask"><?= st($myarray["phone"]); ?></span>
                                ...
                                <td valign="top">
                                        <?= st($myarray["patfirstname"]." ".$myarray["patlastname"]); ?>
                                </td>
				<td valign="top">
					<a href="#" <?= (count($sim))?'class="plus_count"':''; ?> onclick="$('.sim_<?= $myarray['id']; ?>').toggle();return false;"><?= count($sim); ?> <?= (count($sim)>0)?'(click to view)':'';?></a>
				</td>
				<td valign="top">
					<a href="#" onclick="loadPopup('view_patient_insurance.php?id=<?= $myarray["id"]; ?>'); return false;" class="editlink" title="EDIT">
					        View
					</a> 

      ...
      if ($db->query($dsql)) {
        if ($eob['status'] == DSS_CLAIM_DISPUTE) {
          $del_pay = "DELETE FROM dental_ledger_payment 
                      ...
                      ledgerid in (SELECT ledgerid FROM dental_ledger dl WHERE dl.primary_claim_id=".mysql_real_escape_string($_GET['claimid']).")";
...
<?php
  ...
  $pat = $db->getRow($psql);
?>
  <span style="float:right; font-size: 26px; margin-right: 20px; font-weight: bold; color:#f00;">Claim <?php echo  $_GET['claimid']." - ".$thename; ?></span>
...
<?php echo st($pat_myarray['zip']);?> 
  ...
  <div style="float:left; margin-left:20px;">
    <?php if((($claim['status'] == DSS_CLAIM_PENDING || $claim['status'] == DSS_CLAIM_REJECTED ||$claim['status'] == DSS_CLAIM_DISPUTE)
            && $pat['p_m_dss_file']=='2') || (($claim['status'] == DSS_CLAIM_SEC_PENDING || $claim['status'] == DSS_CLAIM_SEC_REJECTED ||$claim['status'] == DSS_CLAIM_SEC_DISPUTE)
            && $pat['s_m_dss_file']=='2')){ ?>	
              <button onclick="Javascript: window.location='insurance_v2.php?insid=<?php echo $_GET["claimid"];?>&pid=<?php echo $_GET["pid"];?>';" class="addButton mainButton">
    ...
    ?>
        <button onclick="Javascript: window.location='insurance_eligible.php?insid=<?php echo $_GET["claimid"];?>&pid=<?php echo $_GET["pid"];?>';" class="addButton mainButton">
		<?php if($claim['status'] == DSS_CLAIM_REJECTED ||$claim['status'] == DSS_CLAIM_SEC_REJECTED){ ?>
            Refile E-File
		<?php } else { ?>
            E-File
    ...
    <?php } ?>
  	<?php } else { ?>
          <button onclick="Javascript: window.location='insurance_v2.php?insid=<?php echo $_GET["claimid"];?>&pid=<?php echo $_GET["pid"];?>';" class="addButton">
  		      View CMS 1500
          </button>
  	<?php } ?>
  </div>
  ...
  <div align="right" style="clear: right;">
    <button onclick="Javascript: window.location = 'add_ledger_payments.php?cid=<?php echo $_GET["claimid"];?>&pid=<?php echo $_GET['pid'];?>';" class="addButton mainButton">
      Make Payment
    </button>
    &nbsp;&nbsp;
    ...
    <?php
      ...
      if ($api_r['use_eligible_api']==1) {
    ?>
        <button onclick="Javascript: window.location='claim_history.php?cid=<?php echo $_GET["claimid"];?>&pid=<?php echo $_GET['pid'];?>';" class="addButton">
          View History 
        </button>
        &nbsp;&nbsp;
    <?php } ?>
    ...
    <table  class="ledger" width="98%" cellpadding="5" cellspacing="1" bgcolor="#FFFFFF" align="center" >
    	...
    	<tr class="tr_bg_h">
    		<td valign="top" class="col_head  <?php echo  ($_REQUEST['sort'] == 'service_date')?'arrow_'.strtolower($_REQUEST['sortdir']):''; ?>" width="10%">
    			<a href="manage_ledger.php?pid=<?php echo  $_GET['pid'] ?>&sort=service_date&sortdir=<?php echo ($_REQUEST['sort']=='service_date'&&$_REQUEST['sortdir']=='ASC')?'DESC':'ASC'; ?>">Svc Date</a>
    		</td>
      ...
      <tr>
        <td colspan="8">
        	<center>
        		<button onclick="Javascript: window.location='manage_ledger.php?pid=<?php echo  $_GET['pid'];?>';return false;" class="addButton">

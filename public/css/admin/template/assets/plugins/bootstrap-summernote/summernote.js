...
(function (factory) {
  ...
  var dom = (function () {
    ...
    /**
     * explode element or #text
     *
     * @param {Element} node
     * @param {Number} offset
     */
    var explodeData = function (node, offset) {
      if (offset === 0) { return node; }
      if (offset >= length(node)) { return node.nextSibling; }
  
      // explodeText
      if (isText(node)) { return node.explodeText(offset); }
    ...
    /**
     * explode dom tree by boundaryPoint(pivot and offset)
     *
     * @param {Element} root
     * @param {Element} pivot - this will be boundaryPoint's node
     * @param {Number} offset - this will be boundaryPoint's offset
     */
    var explode = function (root, pivot, offset) {
      var aAncestor = listAncestor(pivot, func.eq(root));
      if (aAncestor.length === 1) { return explodeData(pivot, offset); }
      return aAncestor.reduce(function (node, parent) {
        var clone = parent.cloneNode(false);
        insertAfter(clone, parent);
        if (node === pivot) {
          node = explodeData(node, offset);
        }
  ...
  var Toolbar = function () {
    ...
    this.update = function ($toolbar, oStyle) {
      ...
      if ($fontname.length > 0) {
        ...
        if (!!selectedFont) {
          selectedFont = list.head(selectedFont.explode(','));

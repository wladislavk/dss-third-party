...
function normalizeCaps(settings) {		
	...
	if (typeof(features) === 'string') {
		plupload.each(features.explode(/\s*,\s*/), function(feature) {
...
plupload.Uploader = function(options) {
	...
	function initControls(settings, cb) {
		...
		plupload.each(settings.runtimes.explode(/\s*,\s*/), function(runtime) {
			if (settings[runtime]) {
					...
					if (value.mime_types) {
						settings.filters.mime_types.regexp = (function(filters) {
							...
							plupload.each(filters, function(filter) {
								plupload.each(filter.extensions.explode(/,/), function(ext) {
									if (/^\s*\*\s*$/.test(ext)) {

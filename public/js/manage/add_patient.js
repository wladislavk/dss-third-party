...
$(document).ready(function() {
  ...
  insContacts = JSON.parse(insContactsJson);
  ...
  if (getParameterByName('sendPin')) {   
    loadPopupWithClose('patient_access_code/pid/' + patientId, 'add_patient/ed/' + patientId + '/preview/1/addtopat/1/pid/' + patientId);
  }

  $('.s_m_ins_div').hide();
  $('#s_m_address_fields').hide();
});
...
function updateNumber(f){
  ...
  if (typeof insContacts[selectedValue] != 'undefined') {
    document.getElementById(f).innerHTML = insContacts[selectedValue];
  } else {

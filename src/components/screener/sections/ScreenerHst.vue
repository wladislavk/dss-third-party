        ...
        <form class="formEl_a screener">
            ...
            <div class="dp50">
                <div class="sepH_b clear" id="hst_company_id_div">
                    ...
                    <img v-if="company.logo" src="../manage/q_file/{{ company.logo }}" />
                    <br />
                    <input type="radio" id="hst_company_id" name="hst_company_id" v-model="hstCompany" v-bind:value="company.id" /> {{ company.name }}
                    <br /><br />
                </div>
                <div v-for="contact in contactData" v-if="contact.hstColumn === 'left'" class="sepH_b" id="hst_{{ contact.name }}_div">
                    ...
                    <input class="inpt_a {{ contact.class }}" type="text" id="hst_{{ contact.name }}" name="hst_{{ contact.name }}" v-model="contact.value" />
                </div>
            </div>
            <div class="dp50">
            ...
            <a href="#" id="sect7_next" class="fr next btn btn_medium btn_d" v-on:click="onSubmit()">Submit Request</a>
        </form>
    </div>
</template>

 ...
 *  or
 *  let api = $('.sigPad').signaturePad({displayOnly:true})
 *  api.regenerate(sig)
 */
(function ($) {
  function SignaturePad (selector, options) {
     ...
     * @type {Object}
     */
    let settings = $.extend({}, $.fn.signaturePad.defaults, options)
     ...
     * @type {Object}
     */
    let canvas = $(settings.canvas, context)
    ...
    function initDrawEvents (e) {
      ...
      if (touchable) {
        ...
        canvas.bind('mouseleave.signaturepad', function () {
    ...
    function type (val) {
      ...
      while ($(settings.typed, context).width() > element.width) {
        let oldSize = $(settings.typed, context).css('font-size').replace(/px/, '')
    ...
    function validateForm () {
      let valid = true
      let errors = {drawInvalid: false, nameInvalid: false}
      let onBeforeArguments = [context, settings]
      let onErrorArguments = [errors, context, settings]
